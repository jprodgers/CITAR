...
| `city` | `city` | `id`, `name`, `owner`, `x`, `y`, `pop`, `buildings`, `worked`, `locked` and `workable` (tiles as `[x, y]`), `specialists` (count by name, those it has), `focus`, `avoid_growth`, `food` (stored), `yields` (the city's total of each stat: `food` is the surplus), `queue` (item names, the first being built), `progress` (production stored by item), `overflow`, `culture` (stored toward the next tile), `health`, `tiles` (the tiles it owns, as `[x, y]`) |
