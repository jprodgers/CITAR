...
//! (1c-08), the city-states' first great-person gift brought forward (1c-06), and what a one-time
//! effect does to a unit (1c-02, whose upgrades a free upgrade waits for).

use super::cities::borders::{expand_borders, take_ownership};
use super::cities::construction::{add_unit_in_city, complete_construction, place_near};
...
use crate::unique::trigger::{CityScope, OneTimeEffect, TriggerEvent, TriggerSite, UnitEffect};
...
fn apply_one_time(g: &mut Game, id: UniqueId, site: &TriggerSite, note: Option<&str>) -> bool {
    ...
    match effect {
        ...
        OneTimeEffect::Unit(e) => {
            let Some(u) = site.unit.filter(|&u| g.unit(u).is_some()) else { return false };
            unit_effect(g, u, e, &suffix)
        }
    }
}
...
fn free_units(
    ...
    for _ in 0..count.max(0) {
        let made = if near_tile {
            ...
            at.and_then(|at| place_near(g, p, name, at))
...
/// A unit takes a promotion for free (`units.add_promotion(free=True)`, `units.py:222-245`): no
/// experience is spent and `upon being promoted` does not fire; a promotion that only acts is not
/// kept; what the promotion does at once happens.
pub(crate) fn promote_free(g: &mut Game, u: UnitId, pr: PromotionId) {
    let r = g.rules();
    let Some((owner, has)) = g.unit(u).map(|x| (x.owner(), x.promotions.contains(pr))) else {
        return;
    };
    if has {
        return;
    }
    let def = &r.promotions()[pr];
    if !super::core::has_type(r, &def.uniques, UniqueType::SkipPromotion)
    ...
    on_gain(g, &def.uniques, &site, None);
}

/// A unit is taken out of the game.
fn remove_unit(g: &mut Game, u: UnitId) -> bool {
    let gone = g.despawn_unit(u);
    debug_assert!(gone.is_ok(), "a unit the game has is removed: {gone:?}");
    gone.is_ok()
}

/// What a one-time effect does to the unit in context (`triggers.py:339-367`).
fn unit_effect(g: &mut Game, u: UnitId, e: UnitEffect, suffix: &str) -> bool {
    let Some((owner, base, at, hp)) = g.unit(u).map(|x| (x.owner(), x.base, x.tile(), x.hp)) else {
        return false;
    };
    match e {
        UnitEffect::Heal(n) => {
            ...
            if let Some(x) = g.unit_mut(u, UnitTouch::CORE) {
                x.hp = i16::try_from(i32::from(x.hp).saturating_add(n).min(100)).unwrap_or(100);
            }
            true
        }
        UnitEffect::Damage(n) => {
            // units.take_damage (units.py:384-392): a unit at no health dies.
            let left = (i32::from(hp) - n).clamp(0, 100);
            if left <= 0 {
                remove_unit(g, u);
                return true;
            }
            if let Some(x) = g.unit_mut(u, UnitTouch::CORE) {
                x.hp = i16::try_from(left).unwrap_or(0);
            }
            true
        }
        UnitEffect::GainXp(n) => {
            if let Some(x) = g.unit_mut(u, UnitTouch::CORE) {
                x.xp = x.xp.saturating_add(n);
            }
            let who = civ_name(g, owner);
            let what = g.rules().base_units()[base].name.clone();
            let text = format!("{who}'s {what} gained {n} XP{suffix}.");
            let told = Some(PlayerSet::single(owner));
            g.emit(EngineEvent::Ruins, &text, told, Some(at), EventData::default(), &[]);
            true
        }
        // units.free_upgrade (units.py:597-602): upgrades are package 1c-02's.
        UnitEffect::Upgrade | UnitEffect::SpecialUpgrade => {
            pending_or(Porting::Pending("1c-02"), false)
        }
        UnitEffect::GainPromotion(pr) => {
            promote_free(g, u, pr);
            true
        }
        ...
        UnitEffect::Destroyed => remove_unit(g, u),
    }
}
