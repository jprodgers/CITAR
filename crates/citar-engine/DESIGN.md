...
**As built in 1b-06** (§5.12, §6.2, §6.5-6.8, §6.11, §9.2, §9.3, §9.7):
- **Files.** Engine: `game/tiles.rs`, `game/religion.rs` (a city's followers and majority, the reads 1b-08 builds on), `game/cities/{stats, connections, citizens, founding}.rs`, `game/derive/stats.rs`, and in `game/economy.rs` happiness, the empire-wide stats, `stat_map`, `civ_stats`, gold per turn and the commits; the actions `SetCityFocus`, `SetSpecialists` and `WorkTile`. Testkit: `tests/engine/cities.rs` and six rule scripts (`cities_{work_tile, focus, specialists, blockade}`, `economy_{unhappy_growth, happiness_commit}`). Refcheck: `answer/{tile_yields, city_stats}.rs` and four more `civs` paths. Bench: `citar-bench/benches/stats.rs`.
- **Tile yields.** `tiles::compute_tile_yield(g, t, viewer, city, mods, &mut deps)` ports `tiles._tile_stats` and adds to `deps` the classes of every conditional and filter it evaluated. The owner's view of a tile as its own city works it is a `CopyMemo` per tile with its recorded classes beside it (a `Cell<CondDeps>`); any other viewer or city is an entry of a table keyed by `(tile, viewer, city)`, validated alike; a city seen by a civilization other than its owner is computed and not kept, since no rule asks. A tile validates against its own inputs and its six neighbours' (fresh water, the coast, adjacency), its owner, which cities exist, the settings, the viewer's `index` and `stocks` (and `CivIndexFull` with no city), its city's `CityMods` and `civ::cond` of the recorded classes.
...
- **City stats.** A source's stats keep the keys Python's dict held (`Yields { stats, keys: StatMask }`), zeros included, since the answers are compared key by key. `CityHappiness` (`CityParts`: tiles, buildings, specialists, the uniques by source, the happiness list) and `CityStats` are memos per city. Their conditionals' classes are static per ruleset (`StatsDeps::of`, from the 27 unique types the city side reads; the civilization side's likewise), which the memos and the citizen recheck read. The uniques that add to each building (`[stats] from every [x]`, `[n]% [stat] from every [x]` and the rest) are gathered once per city, not once per building (`BuildingUniques`), which took the stats of the late fixture's largest city from about 28 to 7 µs.
- **Happiness and the civilization's stats** (`economy::{compute_happiness, global_stats_from_uniques, stat_map, compute_civ_stats, gold_per_turn}`) are memos per civilization, validated against its own revisions, its supply, its cities' `CityHappiness` (and `CityStats` for the stats), and the global `cities`, `routes`, changed tiles, `alliances`, `religions`, `turn`, `diplo` and settings, with the cities' religions when it founded one, and the conditionals of the civilization side's types. `happiness_seen` is committed at S1 and E1 (`commit_happiness_stage`), once per civilization by the setup stage `happiness`, and once after a Python conversion without flags (keeping the cities Python assigned). A commit flags the civilization's cities for a recheck when it moves the ranking's bands (below 0, below -8), or whenever the ruleset's citizen classes read `HAPPINESS_SEEN`. E2 (`end_turn_rates`) writes `last_gold_rate` (flagging the cities when its sign changes) and adds the turn's culture and faith to the totals. E3's bankruptcy reads the stats afresh after each disbanding (its `Pending("1b-06")` is gone).
- **Connectivity** (`cities::connections`): one flood fill per medium whose visited set only grows, the railroad from the capital, the road from every city reached and a harbour's water from every harbour reached, alternating until neither reaches a new city (§6.11). `connected_cities_naive` ports Python's walk plainly, one search per city and medium, behind `cfg(any(test, feature = "test-ops"))`. The memo per civilization validates against `routes`, `cities`, `owners`, the changed tiles, `diplo` (borders, war), `turn`, its `index` (techs), `city_buildings` (harbours) and the settings.
- **Citizens** (`cities::citizens`). `RankCtx` hoists what the ranking reads per city: the focus, growth terms, construction, `happiness_seen` and the sign of `last_gold_rate`. `assign(g, c, reset)` reads only; a settle's passes call it for the flagged cities in id order (`reassign_flagged`, `Pending::take_recheck_from`), and a city whose assignment takes or releases a tile a sibling could work flags the sibling. The writes of `game::mutate` flag what they concern (§6.7): through `Derived::on` the tiles in range, ownership and the blockade (a military unit placed, removed or changing hands, and war and peace); through `recheck_civs` the civilization-level causes, limited by `StatsDeps::ranks`, the classes the ranking's uniques read. `City::citizens_settled` marks a city this engine has assigned, and `citizens::verify`, the citizen oracle, runs in `verify_caches`: every such city has its citizens where a fresh assignment puts them. A city's worked and locked tiles are sets; a tile a sibling works is not workable (`cities-never-share-a-tile`); a lock on a tile no longer workable is dropped.
- **Tools** `work_tile`, `set_city_focus` and `set_specialists` check, write the city (`CityTouch::WORK`), settle, and render their result from the settled game, so it equals what `inspect` shows after the call. A lock past the city's citizens is refused (`work-tile-refuses-a-lock-past-the-citizens`), and a count that is not a number (`specialist-counts-must-be-numbers`).
...
- **Refcheck.** `tile_yields` is enforced whole, `city_stats` but for its religion paths (`followers` and `majority` answered, `pressure_in` missing until 1b-08), and `civs` adds `happiness`, `civ_stats`, `stat_map` and `gold_per_turn`, clean on the 12 committed states and the 250 of the corpus. Intended: `cities-never-share-a-tile`, `marble-bonus-in-its-own-city`, `religion-ties-by-founding-order` (the 24 corpus states where a city's followers tie) and `gold-per-turn-rounds-a-sum-at-a-half`; `between-stat-scales-by-speed` moves to the script list. The ratchet holds `city_stats`' 160 missing `pressure_in` answers, and `civs` falls from 848 to 392.
- **Scripts** (intended, skipped on Python): `happiness-seen-committed`, `last-gold-rate-written`, `work-tile-refuses-a-lock-past-the-citizens`, `specialist-counts-must-be-numbers`, and `citizens-follow-a-blockade-at-once` (Python took a citizen off a blockaded tile at the end of its city's turn). The citizen oracle holds after every step of every script (gate 4).
- **Properties.** Gate 3: on random worlds of the arena (twelve sites, three civilizations, harbours, road and railroad paths with gaps, techs, contact, open borders and war), the floods, the memo and the plain walk agree for every civilization, and still after random route edits. Gate 5: fifty queries, inspections and refusals between each pair of actions leave every digest what it is without them.
- **Criterion** (laptop, the late fixture `small-continents-normal-s1025/t280`, `cargo bench -p citar-bench --bench stats`): a tile yield hit 5.8 ns; the first read of each of the 141 tiles a civilization owns after a write no cache reads 39 ns (the first of them, which validates the cities' modifiers and the civilization's indexes for the rest, 0.1 to 0.3 µs); a recompute 0.3 µs; the stats of the largest city (pop 41, 37 buildings) 6.5 µs; its citizens at pop 20 assigned in 12.5 µs (over the 10 µs budget, under the hard 30 µs); the connectivity of the civilization with 13 cities 1.8 µs; a settle with nothing pending 3 ns. On a machine under load the medians ran up to twice these. `Game::settle_for_bench` and `Game::unrelated_change_for_bench` exist behind `test-ops` for them.
- **Left to later packages:** religious pressure (1b-08: keeping a city's pressures in the order they arrived removes `religion-ties-by-founding-order`); the production queue and construction (1b-07); a faster assignment for large cities.
