...
- **Files.** Engine: `rules/combat.rs` (`CombatRules`: the `Military` filters of a great general's aura, the great people of the `War` pool, and `Carriers` of the aura and of the malus of adjacent enemies, resolved at load), `game/combat/{mod, combatant, strength, resolve, city, air, nuke, actions}.rs`, `game/conquest.rs` (with `diplomacy.on_city_captured`, which only a capture reads, and `religion.remove_unknown_pantheons` and `is_holy_city`), `units::capture::{capture_civilian_by, plan_return_civilian, return_civilian}`, `Game::next_combat_seq`, and the trace `triggers::take_fired_for_test` (feature `test-ops`: what fired on the thread, for the sites whose effects wait for 1b-08). Testkit: `tests/engine/combat.rs`, thirteen scripts `combat_*`, `agents::fight`. Refcheck: `answer/combat_previews.rs`. Bench: `citar-bench/benches/combat.rs`.
- **A side** is the evaluator's `Combatant` (a unit or a city); `combatant` reads what the rules ask of it. **`combat::setup(g, a, from, d, sweeping) -> CombatSetup`** gathers a fight's numbers once: both sides' `Mods` (a `SmallVec` of `(source name, percent)` in the order first given, the name `economy::source_name` of the unique's source as Python's `_src`), the final strengths and the wounded ratios; `damage_to_defender(rnd)` and `damage_to_attacker(rnd)` are then closed-form. Fight contexts are `strength::fight_ctx` (`_ctx`: the side's owner, city or unit and tile, the tile under attack). `preview_of` gives the preview's numbers (`Preview`), which `preview` renders for the tool and `inspect`. A great general's aura is looked for only on the units that may carry one (`CombatRules::aura`), the adjacent enemies' malus likewise; the terrain uniques are asked only for the bonus's sign, and the wounded uniques only of a wounded unit. The Great General aura is not a per-civilization bitset as §6.11 sketched: the carriers' sets made the search cheap enough.
- **Randomness** (§7.2). `resolve::stream(g, purpose, a, b)` takes the next `combat_seq` and keys `[turn, seq, a, b]`: `Combat` for a fight (the two rolls, the blows by `below(pd + pa)`, a withdrawal's tile, a capture's chance, in that order), `Intercept` for each interception (its chance, then its roll), `InterceptOrder` for an air sweep (the candidates shuffled, then sorted by chance, stably; the sweep's blows come from the same stream), `Nuke` for a detonation (tile by tile, the grid's order). A city's plunder and lost buildings draw from `CaptureGold` and `CaptureBuildings` keyed `[city, turn]`. The blows are played on the sides' hit points held apart and written once (a unit's `hp` under `UnitTouch::CORE`, a city's `health` and `damaged_turn` under `CityTouch::CORE`); a unit at 0 is removed in the same call.
- **Conquest.** `conquer` (the other side's military units and aircraft in the city lost, its civilians captured, the plunder, the opinions, the city to the conqueror as a puppet, recaptured, or as the seat's automatic decisions have it), `move_to_civ`, `plan_fate`/`apply_fate` (annex, puppet, raze, stop razing, liberate) and `liberate` (a founder that was eliminated comes back through `Game::revive_player`). A captured city's locks go (`assign_citizens(reset=True)`); the settle places its citizens.
- **Deliberate differences:** `civilians-under-fire` (a civilian a ranged attack brings to 0 dies, and a fight reports a capture only when a melee unit took the civilian), `may-not-annex-refuses-annexing`, and in refcheck `combat-modifiers-in-ruleset-order` (a unit's promotions are a set). `vs-units-never-matches-a-city` moves to `tests/rules/intended.toml`: no reference state shows it. `upon being defeated` fires for the unit that dies (before it is removed), then `upon losing a [unit]` for its owner (§5.9).
- **Waiting for their packages,** marked where Python had them: a barbarian's sack of a city it beat and a camp attacked (1c-06), the city-states' thanks for kills (1c-06), spies fleeing a captured city (1c-05), eliminations and domination after a loss or a capture (1c-08), the effects of triggers other than a unit's and the great general points of combat (1b-08).
...
- **Kitchen sink** (`tests/engine/combat.rs`): `[n] Strength`, `May attack when embarked`, `No defensive terrain penalty`, `[n] Air Interception Range` (the kitchen-sink interceptor now has a chance to intercept), `May not annex cities`, `Never destroyed when the city is captured`, and the triggers `upon conquering a city`, `upon losing a city`, `upon losing a [unit] unit`, `upon defeating a [unit] unit` (with its unit effect) and `upon being defeated`. Deferred: none.
- **Gates.** 2: damage grows with the attacker's strength (a property over the ratio, the roll and the wounds). 3: each fight, bombardment, interception, sweep and detonation takes one `combat_seq`, and a game saved and loaded between two attacks fights the second alike. 4: the thirteen scripts pass on Python and Rust. Liberation reviving a civilization is an engine test (`conquest::tests`), since Rust eliminates at the round's end (1c-08) where Python did at once.
- **Criterion** (laptop, other packages building): `combat/preview` (`preview_of`: the checks, one setup, four damages) 1.6 µs over the committed fixture with the most fights (budget 1 µs, report-only), its checks 0.4 µs and its setup 1.2 µs; the JSON the tool reports adds about 2 µs. What remains is about ten unique queries per fight, each looking up a unit's profile index; a setup that looked the profiles up once would take it under the budget (a candidate for 1e-03).
