...
- **Replay** (§4.11, gate 3). `Full` decodes the chronicle's frames (a frame that does not decode drops the rest until the next keyframe) and writes each in Python's shape, renumbered into the current ruleset's `improvement_ids` and `feature_ids` by name when its palette differs; `unit_ids` is added for a client that decodes `Delta`, whose frames are the records as stored, base64. The test plays a hundred arena turns with a reload: `Full`'s frames are the frames captured as each round ended, its last is the state's map rebuilt as `record_frame` built it, and `Delta`'s records decode back to the captured frames.
- **Performance** (gate 4, report-only, `benches/views.rs`). A player's view of the small t280 fixture (no corpus state is at turn 300) about 0.6 ms against 1.5 ms; a spectator's of `large-pangaea-normal-s1016/t280` about 2.3 ms against 20 ms; of the synthetic gargantuan state (2,500 units, 400 cities, 16,000 tiles, 2.6 MB of JSON) about 10 ms against the 12 ms target. Two changes got there: the typed views, and `num::round_ndigits` in integers for up to 22 decimals and a result under 2^53 units (checked against the formatted rounding on a million values), which every rule's rounding shares.
