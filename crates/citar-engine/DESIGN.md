...
- **The client view** is written straight to bytes: the tiles as typed rows (their features named as they are written, never collected), units as `UnitView` and cities as `CityEntry` (seen, or remembered with `stale`), the rest as values. `NameMap` writes named pairs as an object in order. The bytes of a view and of the replay go through `views::to_py_json`, whose `PyJson` formatter writes each float as Python's `repr` (`2.0`, `1e+16`, `5e-05`, through `PyFloat`) where `serde_json` writes `1e16` and `5e-5`. A viewer the game lacks sees no tile, unit or city, and nothing panics.
