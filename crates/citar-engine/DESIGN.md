...
- **`Derived`** holds the revisions, the grid, the event name index (a memo on `names` and `cities`), the visibility sets (empty until 1c-01; `reveal_for_test` under `test-ops`) and an empty index. `Derived::on` returns `Reactions`: the cities to flag (the tile's own city for a tile change, both claimants for an owner change, a city's own tiles and owner, all of a player's cities for a seat or a war change, and the territory's city when an enemy military unit enters, leaves or dies there) and the vision sources to mark (`pending::SightSource`). The effect queue holds `Effect::Meet`, drained in `(kind, a, b)` order, once each, at most `EffectQueue::LIMIT` per settle.
