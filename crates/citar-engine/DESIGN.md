...
- **The bound** reads two game memos: `TerrainFloor` (the least governing terrain cost on the map, a city's tile one; on cities and `tile_log`) and `RouteNet` (per tile, the steps to the nearest tile that can start a road step, one with a route and a neighbour with a route, and to the nearest that can start a rail step; a breadth-first search from all of them; on routes, cities and the civilizations' index revisions). `Mover::floors()` gives the least an off-route, road and rail step may cost (embarking and disembarking included off the routes). The bound plays the steps a path must still take at those floors: the first `r` off the routes (`r` the tile's distance to the routes), then roads to the nearest railroad, rail, roads, and the target's own `r` off the routes, up to the hex distance; or the same without rail, or all off the routes, whichever is least. Each reading is consistent (a step never lowers it: the property `the_bound_grows_along_every_step`), so, unlike the sketch above, a tile is never reopened. A tile with a route but no neighbour with one starts no road step, so a lone city does not count: without that, the late fixture's isolated railroad cities made the bound nearly useless.
...
- **Kitchen sink.** `CanUpgrade`, `ReducedEmbarkCost`, `XPForPromotionModifier`, `CanMoveOnWater`, `CannotEmbark`, `FreePromotion` and `TriggerUponPromotion` (with a unit effect) are ported and tested in `tests/engine/units.rs`; `TriggerUponGainingUnit` is matched, its effects other than a unit's waiting for 1b-08.
- **Criterion** (laptop, other builds and a local model running; the medians of the bench's own check). Against the budgets:
  - `astar_small_30`, a mid-game small map (`small-pangaea-raging/t50`, 16 targets 25 to 35 tiles away): 27-29 µs, over the 20 µs budget and under the 3x limit. The other small fixtures, reported by the bench and not gated: 36 µs for `scenario-small-continents-s3001/t61`, and 66 µs for the late `small-continents-normal-s1025/t280`, whose worker's paths bend round bays it could cross only by embarking (a whole turn each way), so the bound, blind to the coast, sits well below the truth and the search expands about 290 tiles, half of them water. A busier run measured 32, 48 and 93 µs. Python took 2.5 ms.
  - `astar_garg_fog`, a new gargantuan game's settler to 16 land targets 50 to 58 tiles away through fog: 69-71 µs (budget 150 µs).
  - `reachable`, the late fixture's units with moves: 3.7-3.8 µs (budget 5 µs).

  A search costs about 200 ns per expanded tile: the tile's look (about 20 ns), the bound, the heap and six neighbours. The route-aware bound, over the single cheapest step it replaced, took the mid-game search from 85 µs to 25 µs; ordering the heap by label cost nothing and fixed the paths. What would take the late case under budget is a bound that sees coasts and detours (per-class landmark distances), a candidate for 1e-03.
