...
- **Criterion** (laptop, other packages building beside it; `cargo bench -p citar-bench --bench religion`, own medians): a gargantuan pangaea (160 by 100) of twelve civilizations with a city on every site, 399 cities, six religions founded, every city given a thousand pressure a citizen toward the religion of the holy city nearest it and five rounds played, so that all 399 follow a religion, as late in a game: one round of pressure, every city's religious turn in id order, 0.23 ms (Criterion's 0.31 ms; budget 1 ms). The first cut measured a state where 23 cities followed a religion; on this one it took 1.2 ms, each neighbour recomputing the majority of the city it read (twice, the second in the spread's validation) and each city walking the grid's buckets. Every city's surroundings asked at a stable revision, 0.07 to 0.09 ms.
