...
- **Left to later packages:** `auto_pick_production` (1c-07); triggers (1b-08); a unit's placement by the real `can_stand` (1c-02); a camp's ownership and teleports when a border takes a tile (1c-06, 1c-02). A requirement's local conditional (`in cities without a [Nuclear Plant]`) records `TILE`, which reads where the city's citizens work, so the city's own list recomputes after its citizens move once such a building's tech is known; a class for the city in context alone would remove it.
...
- **Pressure** (`religion::pressures_from_surroundings`, stage E4's `city_end_turn`). `derive::religion` keeps the cities in buckets of ten columns and rows (`CityNeighbours`, valid while `revs.cities` stands) and each city's spread (`SpreadSource`: its majority when that is a major religion, its reach, and its natural pressure's multipliers with the cities each holds for), a memo per city with the classes its computation recorded. Pressure arrives in most cities every turn but rarely changes which religion they follow, so a spread validates against the major religion the city follows (compared itself, not the city's `religion` and `core` revisions), its `buildings`, its owner's and that religion's founder's `index`, the religions, the settings and its recorded classes; the indexes' resource layers are read only for a ruleset whose resources carry a unique a spread reads. A city asks the cities within the farthest reach any religion could have (`reach`: ten, and whatever `Religion naturally spreads to cities [n] tiles away` could add, whatever its conditionals, a memo on the civilizations' `index` revisions; the shipped enhancer Itinerant Preachers adds three), sorted by id, as Python walked them; a city following no major religion is passed over before its spread is validated, and each within its own reach adds its pressure, in one write of the city. A test holds the grid to Python's walk over every city on the 12 committed states and the 250 of the corpus.
...
- **Criterion** (laptop, other packages building beside it; `cargo bench -p citar-bench --bench religion`, own medians): a gargantuan pangaea (160 by 100) of twelve civilizations with a city on every site, 399 cities, six religions founded and spread for twenty rounds (23 cities following one): one round of pressure, every city's religious turn in id order, 0.51 ms (Criterion's 0.58 ms; budget 1 ms). It was 2.2 ms before the reach became a memo (every civilization's index was validated for each city after each write) and before a spread stopped reading its city's pressures; the first cut, passing over cities that follow no major religion, bought little. Every city's surroundings asked at a stable revision, 0.35 to 0.5 ms, most of it the grid's buckets and the majority of the cities in them.
- **Left to later packages:** the spread actions and the great prophet's and great person's unit actions (1c-04); free upgrades (1c-02, whose `units::health::apply_unit_effect` replaces this package's stand-in for the unit effects at the merge); combat's great person points and a conquered city's pantheons (1c-03); the city-states' great-person gift tick (1c-06).
