...
- **Counts** at this package's head: nextest 994 passed, 1 skipped (`--workspace --all-features`, the corpus on); Python 586 OK (2 skipped); `cargo xtask check` 4 NotPorted, 4 Pending (no turn stage); every ratchet group at 0, `civs` clean on the corpus's 250 too; `cargo doc` with `-D warnings` clean.
