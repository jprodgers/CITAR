        ...
        let ok = match effect {
            ...
            OneTimeEffect::Unit(e) => {
                ...
                match e {
                    ...
                    // A free upgrade (package 1c-02's) replaces the warrior with a new unit of
                    // another kind, or does nothing where it has no upgrade.
                    UnitEffect::Upgrade | UnitEffect::SpecialUpgrade => {
                        let replaced = now.is_none()
                            && units(&g) == units(&before)
                            && g.player_units(ME).any(|x| x.base != was.base && x.hp == was.hp);
                        happened == replaced
                    }
