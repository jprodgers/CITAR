# Frozen copy of citar/bots/basic.py made by citar.lab on 2026-09-18 14:24
...
class BasicBot:
    ...
    def consider_diplomacy(self, g: Game, pid: int, ctx: Optional[dict] = None):
        ...
        if prep and not ctx["wars"]:
            ...
            elif (len(field) >= max(4, min(8, len(ctx["cities"]) // 2 + 2)) or (len(field) >= 2 and mine > theirs * 4))                     and mine > theirs * (1.35 - 0.4 * self.aggression):
