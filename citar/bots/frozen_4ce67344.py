# Frozen copy of citar/bots/basic.py made by citar.lab on 2026-09-18 10:46
...
PANTHEON_PREFS = ["Fertility Rites", "God of Craftsmen", "Religious Settlements", "Tradition", "Ancestor Worship",
...
class BasicBot:
    ...
    def choose_research(self, g: Game, pid: int, ctx: Optional[dict] = None):
        ...
        for t in g.rules.techs:
            ...
            score = v / (cost ** 0.8) * self.rng.uniform(0.9, 1.1)
            if score > best_v:
    ...
    def empire_choices(self, g: Game, pid: int, ctx: dict):
        ...
        p = g.player(pid)
        order = POLICY_ORDER["aggressive" if self.aggression > 0.55 else "peaceful"]
        for _ in range(4):
        ...
        if g.religion_enabled and religion.can_found_pantheon(g, pid) is None:
            ...
            pick = next((b for b in PANTHEON_PREFS if b in avail), avail[0] if avail else None)
            if pick:
    ...
    def _manage_city_tiles(self, g: Game, pid: int, c, ctx: dict):
        ...
        if c.razing or c.puppet:
            return
        if c.focus not in ("balanced", "gold") and ctx["gpt"] >= 0:
    ...
    def _choose_production(self, g: Game, pid: int, c, ctx: dict, counts: dict, army: int, danger: bool) -> Optional[str]:
        ...
        if defender:
            ...
            if army < target and (affordable or ctx["offense"]):
                ...
                attacker = (self.best_military(g, c, units, offense=True, melee_only=True) if want_melee else None)                     or (self.best_military(g, c, units, siege_only=True) if want_siege else None) \
        ...
        if settler and any(g.grid.distance(s, c.idx) <= 12 for s in sites) and n + counts["settler"] < max_cities \
                and counts["settler"] < max(1, n // 3) and not danger and (ctx["hap"] >= 2 or (ctx["hap"] >= 0 and n < 3)):
            options.append(((130 if g.turn < 90 else 75) - n * 6, settler))
        met_anyone = any(g.player(q).kind == "major" for q in g.player(pid).met)
        ...
        if not met_anyone and g.turn > 30 and counts["recon"] == 0 and army <= len(ctx["military"])                 and not any(x.activity == "explore" for x in ctx["units"]):
        ...
        if boat and counts["boat"] == 0 and g.turn - self._boat_turn.get(c.id, -99) > 25:
            ...
            for idx in g.grid.within(c.idx, 6):
                ...
                if t.owner == pid and t.resource and g.is_water(idx) and not t.improvement \
                    ...
                    break
        for b in items["buildings"] + items["wonders"]:
    ...
    def _building_value(self, g: Game, pid: int, c, b: str, ctx: dict) -> float:
        ...
        v = sum(diff[k] * STAT_WEIGHTS[k] for k in diff)
        dh = hap - base_hap
        want = len(ctx["cities"]) + 2
        v += dh * STAT_WEIGHTS["happiness"] * (3.0 if ctx["hap"] < 0 else (1.8 if ctx["hap"] < want else 0.4))
        ...
        v += 0.5 * len(bd.get("uniques", []))           # effects the simulation cannot see (free units, XP, ...)
        if bd.get("isWonder"):
            v = v * 1.2 + 2
    ...
    def manage_gold(self, g: Game, pid: int, ctx: dict):
        ...
        if p.gold > 350 + 80 * ctx["era"] and not ctx["wars"]:
    ...
    def _spend_faith(self, g: Game, pid: int, ctx: dict):
        from ..engine import cities as cm, religion
        p = g.player(pid)
    ...
    def handle_settler(self, g: Game, pid: int, u, ctx: dict):
        ...
        if not ctx["cities"] and cm.found_check(g, pid, u.idx) is None:
            ...
            return
        danger = any(g.grid.distance(e.idx, u.idx) <= 3 for e in ctx["hostile"])
        sites = self.expansion_sites(g, pid, ctx)
        ...
        if r is None:
            self._bad_sites[(pid, target)] = g.turn
    ...
    def handle_special(self, g: Game, pid: int, u, ctx: dict):
        ...
        if "found_religion" in acts:
            if "found_religion" in ok:
                ...
                beliefs = religion.ai_choose_beliefs(g, pid, need)
    ...
    def handle_military(self, g: Game, pid: int, u, ctx: dict):
        ...
        if ruins is not None and self._move(g, pid, u, ruins):
            return
        p = g.player(pid)
        if g.turn > 30 and not any(g.player(q).kind == "major" for q in p.met) \
    ...
    def consider_diplomacy(self, g: Game, pid: int, ctx: Optional[dict] = None):
        ...
        for q in list(p.met):
            ...
            if g.turn > 50 and not ctx["wars"] and pid not in self._war_prep and rel["treaty_until"] < g.turn \
                ...
                if mine > theirs * (1.35 - 0.5 * self.aggression) and self.rng.random() < 0.02 + 0.15 * self.aggression \
        ...
        if prep and not ctx["wars"]:
            ...
            elif len(field) >= max(4, len(ctx["cities"])) and mine > theirs * (1.35 - 0.4 * self.aggression):
                if self.ex(g, pid, "declare_war", player_id=q, message="Your lands will be ours.") is not None:
                    self._war_plan[pid] = {"city": target.idx, "since": g.turn, "advance": False, "checked": -1}
                self._war_prep.pop(pid, None)
        elif prep:
            self._war_prep.pop(pid, None)
        if g.turn % 7 == pid % 7:
            ...
            for it in items:
                ...
                if t == "gold":
                    ...
                    if rd["resourceType"] == "Luxury":
                        ...
                        v += sign * (45 if have <= 0 else 12) * it.get("amount", 1)
                elif t == "open_borders":
                    v += sign * 12
                elif t == "embassy":
                    v += sign * 8
