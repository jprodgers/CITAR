# Frozen copy of citar/bots/basic.py made by citar.lab on 2026-09-22 16:41
"""The standard rule-based player: the sparring partner for AI models.

It plays exclusively through the public tool registry (like any AI would), so it also exercises the API. Each turn it
builds a picture of the empire (threats to each city, army size, economy, expansion room) and then:

  * researches toward the most valuable technology for its situation (beeline scoring over prerequisite paths),
  * adopts social policies, founds a pantheon and religion, uses great people and free picks,
  * bombards enemies with its cities, fights with favorable odds, answers threats and clears barbarian camps,
  * keeps settlers and workers out of danger, expands while happiness allows,
  * chooses production by need; buildings are valued like UnCiv's ConstructionAutomation, by simulating the city's
    stats with the building added,
  * keeps gold positive, buys what's urgent, upgrades units, courts city-states, sends spies,
  * wages wars it can win, makes peace when a war goes badly, and trades surplus luxuries.

`aggression` (0-1) shifts army size and willingness to start wars. Every other number the bot uses is a named
parameter in ``PARAM_GROUPS`` (label, explanation and sensible range included), so a bot profile can change any of
them without touching code - which is what the Bots page edits and what lab experiments A/B test.
"""
from __future__ import annotations

import random
from typing import Optional

from ..engine import tools
from ..engine import unique_types as U
from ..engine.game import Game, ActionError

STAT_KEYS = ("food", "production", "gold", "science", "culture", "faith", "happiness")
POLICY_BRANCHES = ["Tradition", "Liberty", "Honor", "Piety", "Patronage", "Commerce", "Rationalism", "Freedom",
                   "Order", "Autocracy"]
POLICY_ORDER = {
    "peaceful": ["Tradition", "Liberty", "Piety", "Patronage", "Rationalism", "Commerce", "Freedom", "Order", "Honor",
                 "Autocracy"],
    "aggressive": ["Honor", "Tradition", "Liberty", "Commerce", "Rationalism", "Autocracy", "Patronage", "Piety",
                   "Order", "Freedom"],
}
POLICY_ORDERS = {      # named alternatives for policy_order_peaceful / _aggressive
    "default": None,
    "rationalism_early": ["Tradition", "Liberty", "Rationalism", "Patronage", "Commerce", "Piety", "Freedom", "Order",
                          "Honor", "Autocracy"],
    "liberty_first": ["Liberty", "Tradition", "Rationalism", "Commerce", "Patronage", "Piety", "Freedom", "Order",
                      "Honor", "Autocracy"],
}
BELIEF_PREFS = {       # belief_mode "prefs": ranked choices (happiness and growth first); unlisted ones come last
    "Pantheon": ["Goddess of Love", "Fertility Rites", "Religious Settlements", "God of Craftsman", "Sacred Waters",
                 "Ancestor Worship", "Monument to the Gods", "Goddess of the Hunt", "Stone Circles", "Oral Tradition"],
    "Founder": ["Ceremonial Burial", "Tithe", "Church Property", "Peace Loving", "Pilgrimage", "Initiation Rites",
                "Interfaith Dialogue", "World Church", "Papal Primacy"],
    "Follower": ["Pagodas", "Religious Center", "Feed the World", "Mosques", "Cathedrals", "Religious Community",
                 "Swords into Ploughshares", "Peace Gardens", "Divine inspiration", "Guruship", "Asceticism",
                 "Choral Music", "Liturgical Drama", "Monasteries", "Religious Art", "Holy Warriors"],
    "Enhancer": ["Religious Texts", "Itinerant Preachers", "Missionary Zeal", "Reliquary", "Messiah", "Holy Order",
                 "Just War", "Religious Unity", "Defender of the Faith"],
}
PANTHEON_PREFS = ["Fertility Rites", "God of Craftsman", "Religious Settlements", "Tradition", "Ancestor Worship",
                  "Goddess of Love", "Monument to the Gods", "God of War", "Messenger of the Gods"]
PROMOTION_LINES = ["Shock", "Drill", "Accuracy", "Barrage", "Bombardment", "Targeting", "Interception"]
GREAT_PEOPLE = ["Great Scientist", "Great Engineer", "Great Merchant", "Great Artist", "Great Writer",
                "Great Musician", "Great Prophet", "Great General", "Great Admiral"]


# ----------------------------------------------------------------------------------------------------------------------
# Parameters. Every number the bot decides with is here, with what it means, so a profile can change any of it.
# Types matter: an int default stays an int (it is used in integer arithmetic and comparisons), and divisors stay
# divisors (x / 25 and x * 0.04 are not the same float).
# ----------------------------------------------------------------------------------------------------------------------
def _n(key, default, label, help="", lo=None, hi=None, unit=""):
    """A numeric parameter."""
    return {"key": key, "default": default, "type": "int" if isinstance(default, int) and not isinstance(default, bool)
            else "float", "label": label, "help": help, "min": lo, "max": hi, "unit": unit}


def _b(key, default, label, help=""):
    """An on/off parameter."""
    return {"key": key, "default": default, "type": "bool", "label": label, "help": help}


def _c(key, default, label, help, choices):
    """A choice among named values (None is allowed when it is listed)."""
    return {"key": key, "default": default, "type": "choice", "label": label, "help": help, "choices": choices}


def _o(key, default, label, help, options, presets=None, ordered=True):
    """A list of names: an order of preference (ordered) or a set to pick from. `presets` are named alternatives that
    the value may also be given as (a string); None means the built-in default."""
    return {"key": key, "default": default, "type": "order" if ordered else "list", "label": label, "help": help,
            "options": options, "presets": presets or {}}


PARAM_GROUPS: list[tuple[str, str, list[dict]]] = [
    ("Personality", "Temperament switches. Aggression itself (0-1) is set per profile or per seat.", [
        _n("aggressive_above", 0.55, "Aggressive above", "Aggression above this uses the aggressive policy order.",
           0, 1),
        _n("war_prep_rate", 1.0, "War preparation rate", "Multiplier on the per-turn chance of starting to prepare "
           "a war against a weaker neighbour.", 0, 5, "x"),
    ]),
    ("Research", "How technologies are valued. The bot beelines the best value per cost over the prerequisite path.", [
        _c("tech_mode", "classic", "Valuation mode", "classic: flat building stats; potential: estimated "
           "empire-wide yield of what the tech unlocks.", ["classic", "potential"]),
        _n("tech_cost_exp", 0.8, "Cost exponent", "Score = value / cost^this. Lower favours expensive, valuable "
           "techs.", 0, 1.5),
        _n("tech_path_value", 0.5, "Prerequisite value share", "Share of each prerequisite's own value added to "
           "the goal.", 0, 1),
        _n("tech_noise", 0.1, "Randomness", "Scores are multiplied by a random factor in 1 +- this.", 0, 0.5),
        _n("tech_base_value", 1.0, "Base value", "Value every tech starts with.", 0, 10),
        _n("tech_building_base", 3, "Per building unlocked", "Classic mode: flat value per building unlocked.", 0, 20),
        _n("tw_food", 1.2, "Building food weight", "Classic mode: weight of a building's food.", 0, 5),
        _n("tw_production", 1.3, "Building production weight", "Classic mode.", 0, 5),
        _n("tw_gold", 0.9, "Building gold weight", "Classic mode.", 0, 5),
        _n("tw_science", 1.1, "Building science weight", "Classic mode.", 0, 5),
        _n("tw_culture", 0.8, "Building culture weight", "Classic mode.", 0, 5),
        _n("tw_faith", 0.6, "Building faith weight", "Classic mode.", 0, 5),
        _n("tw_happiness", 1.8, "Building happiness weight", "Classic mode.", 0, 5),
        _n("tech_building_unique", 1, "Per building rule", "Classic mode: value per special rule of a building.", 0, 10),
        _n("tech_wonder", 2, "Wonder bonus", "Classic mode: extra value for unlocking a wonder.", 0, 20),
        _n("pot_base", 2, "Potential: per building", "Potential mode: flat value per building unlocked.", 0, 20),
        _n("pot_specialist", 0.5, "Potential: per specialist slot", "Potential mode.", 0, 5),
        _n("tech_default_power", 8, "Default unit strength", "Strength assumed when the civ has no land military "
           "unit yet (to judge upgrades).", 1, 50),
        _n("tech_civilian_unit", 3, "Per civilian unit", "Value of unlocking a non-military unit.", 0, 20),
        _n("tech_unit_min_gain", 1.05, "Unit upgrade threshold", "A new military unit counts only when it is this "
           "many times stronger than the best we have.", 1, 2, "x"),
        _n("tech_unit_base", 4, "Stronger unit: base", "Value of a stronger military unit ...", 0, 50),
        _n("tech_unit_gain", 8, "Stronger unit: per strength gain", "... plus this per 100% strength gain ...", 0, 50),
        _n("tech_unit_war_mult", 2.2, "Stronger unit: at war", "... times this while at war ...", 1, 5, "x"),
        _n("tech_unit_aggr_base", 0.6, "Stronger unit: aggression base", "... times (this + aggression).", 0, 2),
        _n("tech_improvement", 3, "Per improvement", "Value per tile improvement unlocked.", 0, 20),
        _n("tech_reveal", 3, "Per resource revealed", "Value per resource revealed.", 0, 20),
        _n("tech_lux_needed", 14, "New luxury (needed)", "Improving a luxury we own none of while happiness is "
           "short.", 0, 50),
        _n("tech_lux", 7, "New luxury", "Improving a luxury we own none of otherwise.", 0, 50),
        _n("tech_lux_hap_margin", 3, "Happiness is short below", "Happiness counts as short below (cities + this).",
           -10, 20),
        _n("tech_strategic", 3, "Strategic resource", "Improving a strategic resource in our borders.", 0, 20),
        _n("tech_unique", 2, "Per tech rule", "Value per special rule of the tech itself.", 0, 20),
        _n("tech_space_era", 5, "Space race from era", "Era from which spaceship techs get a bonus.", 0, 8),
        _n("tech_space", 25, "Spaceship tech bonus", "Bonus for techs unlocking spaceship parts or projects.", 0, 100),
    ]),
    ("Building value", "What cities build. UnCiv mode values every option and builds the best value per remaining "
     "production; classic mode uses fixed priorities. Building values come from simulating the city with the "
     "building added and weighting the difference.", [
        _c("prod_mode", "unciv", "Production mode", "unciv: UnCiv's ConstructionAutomation; classic: fixed "
           "priorities.", ["unciv", "classic"]),
        _n("u_food", 3.6, "Food", "UnCiv mode: value per point of food a building adds.", 0, 10),
        _n("u_production", 2.0, "Production", "UnCiv mode: per point of production.", 0, 10),
        _n("u_gold", 0.67, "Gold", "UnCiv mode: per point of gold.", 0, 10),
        _n("u_science", 2.0, "Science", "UnCiv mode: per point of science.", 0, 10),
        _n("u_culture", 1.0, "Culture", "UnCiv mode: per point of culture.", 0, 10),
        _n("u_faith", 1.0, "Faith", "UnCiv mode: per point of faith.", 0, 10),
        _n("u_happiness", 1.0, "Happiness", "UnCiv mode: per point of happiness.", 0, 10),
        _n("u_happiness_low", 6.0, "Happiness multiplier when low", "Happiness weight multiplier while happiness is "
           "low (UnCiv: x3).", 1, 20, "x"),
        _n("u_happiness_low_below", 10, "Happiness is low below", "Low means below this, or below the number of "
           "cities.", -10, 50),
        _n("u_gold_broke_mult", 3, "Gold multiplier when broke", "Gold weight multiplier with negative gold and "
           "income.", 1, 10, "x"),
        _n("u_culture_first_mult", 2, "First culture multiplier", "Culture weight multiplier in a city with little "
           "culture.", 1, 10, "x"),
        _n("u_culture_first_below", 2, "Little culture below", "A city has little culture below this per turn.",
           0, 20),
        _n("u_carryover_share", 0.5, "Food carry-over", "Granary-style food kept on growth: value share of the food "
           "saved.", 0, 2),
        _n("u_gpp", 1.5, "Great person points", "Per great person point a building adds.", 0, 5),
        _n("u_specialist", 0.5, "Specialist slot", "Per specialist slot a building adds.", 0, 5),
        _n("u_defense_war", 1.0, "Defence at war", "Walls/castle value factor while at war ...", 0, 5),
        _n("u_defense_peace", 0.5, "Defence at peace", "... and at peace ...", 0, 5),
        _n("u_defense_threat_mult", 2, "Defence under threat", "... doubled (this) when enemies are near the city.",
           1, 10, "x"),
        _n("u_city_health", 4, "City health", "Value of added city hit points, relative to current maximum.", 0, 20),
        _n("u_city_strength", 4, "City strength", "Value of added city strength, relative to current.", 0, 20),
        _n("u_city_strength_offset", 3, "City strength offset", "Added to current strength in that ratio.", 0, 20),
        _n("u_wonder_bonus", 12.0, "Wonder bonus", "Flat value added to a world wonder.", 0, 50),
        _n("u_national_wonder_share", 0.5, "National wonder share", "Share of the wonder bonus a national wonder "
           "gets.", 0, 1),
        _b("u_wonder_gate", False, "Gate wonders", "UnCiv: wonders only in above-average-production cities of an "
           "empire with enough population."),
        _n("wonder_gate_pop", 12, "Wonder gate population", "Empire population needed for wonders when gated.",
           0, 100),
        _n("u_victory_building", 1500, "Victory building", "Value of a building that wins the game (e.g. Utopia).",
           0, 5000),
    ]),
    ("Build priorities", "UnCiv mode: the value of units and projects against buildings, and when each is wanted.", [
        _n("u_settler", 30.0, "Settler", "Value of a settler.", 0, 200),
        _n("settler_site_range", 12, "Settler site range", "Only build settlers when a site is this close.", 1, 30,
           "tiles"),
        _n("settler_per_cities", 3, "Settlers per cities", "At most one settler in flight per this many cities "
           "(at least one).", 1, 20),
        _n("settler_min_pop", 2, "Settler minimum size", "A city must be at least this size to build a settler.",
           1, 10),
        _n("settler_min_hap", 2, "Settler minimum happiness", "Build settlers while happiness is at least this ...",
           -20, 20),
        _n("settler_min_hap_small", 0, "... with a small empire", "... or at least this with a small empire.",
           -20, 20),
        _n("settler_small_empire", 3, "Small empire below", "Empire counts as small below this many cities.", 1, 20),
        _n("u_worker", 1.0, "Worker", "Value of a worker (scaled by how many are missing).", 0, 20),
        _n("workers_per_city", 1.8, "Workers per city", "Target workers per city (UnCiv).", 0, 5),
        _n("workers_full_until", 5, "Full worker target up to", "Up to this many cities the full target applies ...",
           1, 30),
        _n("workers_extra_share", 0.72, "Worker target after that", "... then this share of it per extra city.",
           0, 2),
        _n("worker_count_offset", 0.17, "Worker urgency offset", "Value = want / (workers + this): smaller makes the "
           "first worker more urgent.", 0.01, 5),
        _n("worker_unimproved", 0.0, "Workers per unimproved tile", "Extra wanted workers per worked, unimproved "
           "land tile.", 0, 1),
        _n("u_boat", 6.0, "Work boat", "Value of a work boat when a sea resource needs one.", 0, 50),
        _n("boat_search_radius", 6, "Work boat search", "Sea resources this close to a city count.", 1, 15, "tiles"),
        _n("u_scout", 4.0, "Scout", "Value of a scout while no rival city is known.", 0, 50),
        _n("scout_until_turn", 150, "Scouts until turn", "No scouts are built after this turn.", 0, 500, "turn"),
        _n("u_seeker", 6.0, "Explorer", "Value of a unit sent to find rivals when still alone.", 0, 50),
        _n("seek_after_turn", 30, "Look for rivals after", "Still alone after this turn: send units exploring.",
           0, 300, "turn"),
        _n("seek_military_after", 45, "Explore with military after", "After this turn explorers are military "
           "units, not scouts.", 0, 300, "turn"),
        _n("u_military", 1.0, "Military", "Value of a military unit (scaled by need, below).", 0, 20),
        _n("u_offense", 3.0, "Offensive build-up", "Military value multiplier while preparing or fighting a war "
           "with too small an army (times 0.5 + aggression).", 0, 20, "x"),
        _n("mil_base", 1, "Military need: base", "Need = this + sqrt(cities / (army + 1)) x scale.", 0, 10),
        _n("mil_scale", 0.5, "Military need: scale", "", 0, 5),
        _n("mil_war_mult", 2, "Military need at war", "Multiplier while at war.", 1, 10, "x"),
        _n("mil_offense_aggr_base", 0.5, "Offense: aggression base", "Offensive multiplier is u_offense x (this + "
           "aggression).", 0, 2),
        _n("mil_barbarian_min", 2.0, "Barbarians near: minimum", "Military need is at least this with barbarians "
           "in sight.", 0, 20),
        _n("mil_below_avg_div", 5, "Weak city divisor", "Military need is divided by this in below-average "
           "production cities.", 1, 20),
        _n("mil_army_full_div", 3, "Army complete divisor", "Divided by this when the army target is met (at peace).",
           1, 20),
        _n("peace_army_min", 7, "Peacetime army cap", "At peace, stop building military above max(this, cities x "
           "per city) ...", 0, 50),
        _n("peace_army_per_city", 5, "... per city", "", 0, 20),
        _n("military_min_gold", -50, "Military gold floor", "No new military below this much gold.", -1000, 1000,
           "gold"),
        _n("u_project", 40.0, "Project", "Value of a project (Apollo, Manhattan...) in a strong city ...", 0, 500),
        _n("u_project_low", 10.0, "... in a weak city", "", 0, 500),
        _n("u_spaceship", 1500.0, "Spaceship part", "Value of a spaceship part in a strong city.", 0, 5000),
        _n("u_space_program", 1500.0, "Space program", "Extra value of the building that enables spaceship parts "
           "(Apollo Program) in a strong city.", 0, 5000),
        _n("space_reserve", 3, "Keep for the spaceship", "From the space era (see Research), keep this many of each "
           "resource spaceship parts need (Aluminum) free: no units or buildings that would use them. 0 = off.",
           0, 6),
        _n("garrison_after_turn", 12, "Garrison after turn", "An empty city builds a defender first after this turn "
           "(sooner with enemies in sight).", 0, 100, "turn"),
        _n("danger_ratio", 0.5, "City in danger at", "A city is in danger when nearby enemy strength exceeds this "
           "share of its defence.", 0, 5, "x"),
    ]),
    ("Classic production", "Priorities used only when production mode is classic.", [
        _n("c_escort", 140, "Escort for a waiting settler", "", 0, 2000),
        _n("c_danger", 1000, "Defender in danger", "", 0, 2000),
        _n("c_garrison", 300, "Garrison an empty city", "", 0, 2000),
        _n("c_military_min_gpt", 1, "Military needs income", "Peacetime units only with at least this gold per turn.",
           -50, 50),
        _n("c_army_offense", 90, "Army (war): base", "Priority = (base + aggr x aggression) x (short base + short "
           "scale x shortfall).", 0, 500),
        _n("c_army_offense_aggr", 70, "Army (war): per aggression", "", 0, 500),
        _n("c_army_peace", 25, "Army (peace): base", "", 0, 500),
        _n("c_army_peace_aggr", 35, "Army (peace): per aggression", "", 0, 500),
        _n("c_army_short_base", 0.6, "Army shortfall base", "", 0, 5),
        _n("c_army_short_scale", 1.4, "Army shortfall scale", "", 0, 5),
        _n("settler_prio", 130, "Settler (early)", "", 0, 1000),
        _n("settler_prio_late", 75, "Settler (late)", "", 0, 1000),
        _n("settler_prio_until", 90, "Early settlers until turn", "", 0, 500, "turn"),
        _n("settler_prio_per_city", 6, "Settler priority per city", "Subtracted per city owned.", 0, 100),
        _n("c_scout_needed", 80, "Scout (alone)", "Priority while no rival city is known ...", 0, 1000),
        _n("c_scout_after_turn", 15, "... after turn", "", 0, 300, "turn"),
        _n("c_scout", 30, "Scout (otherwise)", "", 0, 1000),
        _n("c_scout_early_turns", 30, "Scouts early until", "Scouts are built until this turn even with contact.",
           0, 300, "turn"),
        _n("c_seeker", 150, "Explorer (alone)", "", 0, 1000),
        _n("c_worker_extra", 1, "Extra workers (small)", "Workers wanted = cities + this for small empires.", 0, 5),
        _n("c_worker_extra_until", 2, "Small empire up to", "", 0, 20),
        _n("c_worker_urgent_div", 2, "Urgent below cities /", "Workers are urgent below cities / this.", 1, 10),
        _n("c_worker_urgent", 100, "Worker (urgent)", "", 0, 1000),
        _n("c_worker", 50, "Worker", "", 0, 1000),
        _n("c_boat", 45, "Work boat", "", 0, 1000),
        _n("c_boat_retry_turns", 25, "Work boat retry", "Turns before another boat is queued in the same city.",
           0, 200, "turns"),
        _n("c_building_scale", 12, "Building scale", "Priority = value x scale / (1 + turns / turns scale).", 0, 100),
        _n("c_building_turns", 12, "Building turns scale", "", 1, 100),
        _n("c_project", 350, "Project", "", 0, 2000),
        _n("c_spaceship", 400, "Spaceship part", "", 0, 2000),
    ]),
    ("Classic building value", "Building values used only in classic production mode.", [
        _n("w_food", 1.2, "Food", "", 0, 10), _n("w_production", 1.3, "Production", "", 0, 10),
        _n("w_gold", 0.9, "Gold", "", 0, 10), _n("w_science", 1.1, "Science", "", 0, 10),
        _n("w_culture", 0.8, "Culture", "", 0, 10), _n("w_faith", 0.6, "Faith", "", 0, 10),
        _n("w_happiness", 1.8, "Happiness", "", 0, 10),
        _n("w_hap_margin", 2, "Happiness wanted", "Happiness is wanted up to cities + this.", -10, 20),
        _n("w_hap_unhappy", 3.0, "Happiness when unhappy", "Happiness multiplier when unhappy ...", 0, 20, "x"),
        _n("w_hap_low", 1.8, "... when short", "", 0, 20, "x"),
        _n("w_hap_ok", 0.4, "... otherwise", "", 0, 20, "x"),
        _n("w_gold_broke", 1.5, "Gold when broke", "Extra per gold with negative income.", 0, 10),
        _n("w_gpp", 1.5, "Great person points", "", 0, 10),
        _n("w_specialist", 1.2, "Specialist slot", "", 0, 10),
        _n("w_def_strength_div", 2, "City strength divisor", "Defence value = strength / this + health / divisor ...",
           1, 100),
        _n("w_def_health_div", 25, "City health divisor", "", 1, 500),
        _n("w_def_border", 3.0, "Defence at war or threatened", "... times this at war or threatened ...", 0, 20),
        _n("w_def_interior", 0.3, "Defence otherwise", "... or this.", 0, 20),
        _n("unique_bonus", 0.5, "Per building rule", "Per rule text the simulation can't see.", 0, 10),
        _n("wonder_mult", 1.2, "Wonder multiplier", "", 0, 10, "x"),
        _n("wonder_bonus", 2.0, "Wonder bonus", "", 0, 50),
        _n("wonder_min_pop", 0, "Wonder empire population", "UnCiv-style gate: wonders only with this much "
           "population.", 0, 100),
        _b("wonder_avg_prod", False, "Wonders in strong cities", "Only in cities producing at least the empire "
           "average."),
        _n("w_small_city_pop", 2, "Small city up to", "", 0, 10),
        _n("w_small_city_mult", 0.8, "Small city multiplier", "Building values in small cities.", 0, 2, "x"),
    ]),
    ("Expansion", "Where and when to found cities, and how settlers travel.", [
        _n("site_min_score", 14, "Minimum site score", "Sites scoring below this are ignored.", 0, 50),
        _n("site_new_lux", 0, "New luxury at site", "Site bonus per luxury type we don't own yet.", 0, 30),
        _n("site_lux_radius", 2, "New luxury within", "Luxuries this close to a site count for that bonus.", 1, 5,
           "tiles"),
        _n("site_distance_cost", 1.5, "Site distance cost", "Subtracted from a site's score per tile from our "
           "nearest city.", 0, 10),
        _n("site_radius", 9, "Site search radius", "", 3, 20, "tiles"),
        _n("site_min_distance", 4, "Minimum distance to our cities", "", 2, 10, "tiles"),
        _n("site_spacing", 4, "Spacing between planned sites", "", 2, 10, "tiles"),
        _n("site_candidates", 6, "Sites planned", "How many sites are kept in the plan.", 1, 20),
        _n("site_cache_turns", 4, "Replan every", "", 1, 20, "turns"),
        _n("site_blacklist_turns", 30, "Avoid failed sites for", "", 0, 200, "turns"),
        _n("settler_rank_cost", 1.5, "Settler: rank cost", "A settler picks min(distance + rank x this).", 0, 10),
        _n("settler_found_here_dist", 10, "Found in place within", "With no site, found where standing if within "
           "this of our cities.", 0, 30, "tiles"),
        _n("settler_danger_margin", 1, "Settler danger margin", "An enemy within its reach + this is a danger.",
           0, 5, "tiles"),
        _n("settler_max_retreats", 3, "Retreats before giving up a site", "", 1, 20),
        _n("escort_min_hp", 50, "Escort minimum health", "", 0, 100, "hp"),
        _n("escort_radius", 6, "Escort search radius", "", 1, 20, "tiles"),
        _n("target_cities", 0, "Target cities", "Stop founding cities at this many (0 = no limit).", 0, 50),
    ]),
    ("Cities", "Citizens, focus and growth.", [
        _c("small_city_focus", None, "Small city focus", "e.g. food: non-capital cities below the size below grow "
           "first.", [None, "food", "production", "gold", "science"]),
        _n("small_city_pop", 8, "Small city below", "", 1, 30),
        _b("unhappy_avoid_growth", False, "Stop big cities growing when unhappy", "While unhappy (-75% growth "
           "everywhere), the biggest cities stop growing so the rest grow normally."),
        _n("avoid_growth_div", 4, "Biggest 1 in", "The biggest 1 in this many cities stop growing.", 1, 20),
        _n("avoid_growth_min_pop", 6, "... when at least size", "", 1, 30),
        _n("avoid_growth_release", 2, "Resume growth at happiness", "", -10, 20),
        _n("bv_cache_turns", 0, "Reuse building values for", "Turns a city's building values are reused while it "
           "is unchanged (0 = always simulate). Speed only.", 0, 20, "turns"),
    ]),
    ("Army", "How big the army is and which units it builds.", [
        _n("army_per_city", 1.0, "Army per city", "Army target = cities x (this + aggr x aggression) + base ...",
           0, 5),
        _n("army_per_city_aggr", 0.6, "... per city per aggression", "", 0, 5),
        _n("army_base", 1, "... + base", "", 0, 20),
        _n("army_war_per_city", 1, "At war: + per city", "... + cities x this + extra while at war or preparing ...",
           0, 5),
        _n("army_war_extra", 3, "At war: + extra", "", 0, 20),
        _n("army_per_threatened_city", 1, "Per threatened city", "... + this per threatened city.", 0, 5),
        _n("siege_per_units", 3, "Siege: one per", "When attacking, one siege unit per this many field units.",
           1, 20),
        _n("min_field_melee", 2, "Minimum melee in the field", "Only melee units capture cities.", 0, 10),
        _n("ranged_chance", 0.3, "Chance to prefer ranged", "", 0, 1),
        _n("unit_ranged_pref", 1.15, "Ranged when preferred", "Ranged strength multiplier when ranged is preferred ...",
           0, 3, "x"),
        _n("unit_ranged_nopref", 0.85, "... otherwise", "", 0, 3, "x"),
        _n("unit_siege_defensive", 0.5, "Siege when defending", "Value of siege units when not attacking.", 0, 2, "x"),
        _n("unit_mobile_defensive", 0.8, "Mounted/armor when defending", "", 0, 2, "x"),
        _n("unit_cost_exp", 0.35, "Unit cost exponent", "Unit value = strength / cost^this.", 0, 1.5),
        _o("promo_in_city", ["Cover"], "Promotions in a city", "Promotion lines taken by units standing in a city.",
           PROMOTION_LINES + ["Cover"], ordered=False),
        _o("promo_lines", PROMOTION_LINES, "Promotion lines", "Promotion lines taken otherwise (else the first "
           "available).", PROMOTION_LINES + ["Cover"], ordered=False),
    ]),
    ("Tactics", "Fighting, healing, garrisons, sieges.", [
        _n("threat_radius", 4, "Threat radius", "Enemy units this close count as a threat to a city.", 1, 10, "tiles"),
        _n("threat_near_dist", 2, "Close threat within", "", 0, 10, "tiles"),
        _n("threat_near_weight", 1.0, "Close threat weight", "", 0, 5),
        _n("threat_far_weight", 0.6, "Distant threat weight", "", 0, 5),
        _n("atk_city_loss_weight", 1.6, "City attack: damage taken weight", "Attack value = damage dealt - damage "
           "taken x this.", 0, 5),
        _n("atk_siege_city_mult", 1.5, "Siege unit vs city", "Damage dealt multiplier for siege units attacking "
           "a city.", 0, 5, "x"),
        _n("atk_city_min_hp", 40, "City attack: keep health", "Melee units don't attack a city if left below this.",
           0, 100, "hp"),
        _n("atk_unit_loss_weight", 1.3, "Unit attack: damage taken weight", "", 0, 5),
        _n("atk_kill_mult", 3, "Kill multiplier", "Damage dealt counts this many times when it kills.", 1, 10, "x"),
        _n("atk_unit_min_hp", 25, "Unit attack: keep health", "", 0, 100, "hp"),
        _n("siege_city_pref", 3.0, "Focus on the besieged city", "Attack value multiplier on the city under siege.",
           0, 10, "x"),
        _b("siege_move_first", True, "Siege units move first", "Ranged units in a siege move into range and sight "
           "of the city before shooting."),
        _n("siege_approach_extra", 3, "Siege approach range", "Ranged units within range + this close in.", 0, 10,
           "tiles"),
        _n("heal_below_hp", 50, "Heal below", "", 0, 100, "hp"),
        _n("heal_city_radius", 4, "Heal in a city within", "", 0, 20, "tiles"),
        _n("naval_heal_hp", 60, "Ships heal below", "", 0, 100, "hp"),
        _c("garrison_mode", "all", "Garrisons", "all: every city keeps a unit (the whole army can end up in cities); "
           "exposed: only the capital and cities near a foreign city, a barbarian camp or enemies in sight.",
           ["all", "exposed"]),
        _n("garrison_exposed_radius", 6, "Exposed within", "A foreign city or barbarian camp this close makes a city "
           "exposed.", 1, 20, "tiles"),
        _n("garrison_radius", 10, "Garrison empty cities within", "", 0, 30, "tiles"),
        _n("defend_radius", 8, "Defend a city within", "", 0, 30, "tiles"),
        _n("defend_radius_campaign", 5, "... while campaigning", "", 0, 30, "tiles"),
        _n("campaign_defend_ratio", 0.4, "Campaign: defend when", "While campaigning, only turn back when threat "
           "exceeds this share of the city's defence.", 0, 5, "x"),
        _n("defend_distance_cost", 2, "Defence distance cost", "Pick the city with most threat - distance x this.",
           0, 10),
        _n("camp_city_radius", 14, "Clear camps near our cities", "", 0, 40, "tiles"),
        _n("camp_unit_radius", 16, "Clear camps within reach", "", 0, 40, "tiles"),
        _n("ruin_radius", 5, "Ancient ruins within", "", 0, 20, "tiles"),
        _n("approach_fallback_radius", 3, "Approach fallback", "", 1, 10, "tiles"),
        _n("general_front_radius", 4, "Generals follow the front", "Enemies within this of a unit mark the front.",
           0, 20, "tiles"),
        _n("scout_disband_turn", 60, "Disband idle scouts after", "", 0, 500, "turn"),
        _n("boat_seek_radius", 8, "Work boats look within", "", 1, 30, "tiles"),
        _n("spare_obsolete_mult", 0.5, "Obsolete unit value", "When disbanding to save gold.", 0, 1, "x"),
        _n("spare_garrison_value", 30, "Garrison value", "Added to a garrison's value when choosing what to disband.",
           0, 200),
        _n("keep_units_per_city", 1, "Keep units per city", "Never disband below this many per city.", 0, 5),
    ]),
    ("War", "When to start a war, how to gather, when to advance.", [
        _n("war_min_turn", 50, "No wars before turn", "", 0, 500, "turn"),
        _n("war_max_threat", 0.15, "Only when safe", "Start only while total threat is below this share of our "
           "power.", 0, 2),
        _n("war_power_ratio", 1.35, "Start: power ratio", "Prepare against a neighbour when our power > theirs x "
           "(ratio - aggr x aggression).", 0.5, 5, "x"),
        _n("war_power_ratio_aggr", 0.5, "Start: aggression discount", "", 0, 2),
        _n("war_chance", 0.02, "Start: chance per turn", "Chance per turn per candidate = (chance + aggr x "
           "aggression) x war preparation rate.", 0, 1),
        _n("war_chance_aggr", 0.15, "Start: chance per aggression", "", 0, 1),
        _n("war_prep_timeout", 40, "Give up preparing after", "", 1, 200, "turns"),
        _n("war_abort_ratio", 1.15, "Abort below power ratio", "Stop preparing when power < theirs x (ratio - aggr x "
           "aggression).", 0.5, 5, "x"),
        _n("war_abort_ratio_aggr", 0.4, "Abort: aggression discount", "", 0, 2),
        _n("war_need_min", 4, "Declare: minimum army", "Field units needed = clamp(cities / div + base, min, max).",
           0, 30),
        _n("war_need_max", 8, "Declare: maximum needed", "", 0, 30),
        _n("war_need_city_div", 2, "Declare: cities divisor", "", 1, 10),
        _n("war_need_base", 2, "Declare: base", "", 0, 10),
        _b("prep_gather", True, "Gather before declaring", "Assemble the field army at a rally point before "
           "declaring, then advance at once."),
        _n("prep_gather_radius", 3, "Gathered within", "", 0, 10, "tiles"),
        _n("overwhelm_units", 2, "Overwhelming: units", "Declare with at least this many units ...", 0, 20),
        _n("overwhelm_ratio", 4, "Overwhelming: power ratio", "... when our power exceeds theirs this many times.",
           1, 20, "x"),
        _n("declare_ratio", 1.35, "Declare: power ratio", "Declare when power > theirs x (ratio - aggr x "
           "aggression).", 0.5, 5, "x"),
        _n("declare_ratio_aggr", 0.4, "Declare: aggression discount", "", 0, 2),
        _n("war_target_max_dist", 99, "Target within", "Only attack cities this close to one of ours; with none in "
           "reach, a war is fought defensively (99 = anywhere on our continent).", 3, 99, "tiles"),
        _b("war_overseas", False, "Wars overseas", "Once units can embark (Optics), rival cities on other continents "
           "count as targets; the rally point is on the target's continent, so the army sails there."),
        _c("war_target_pick", "smallest", "Chosen war target", "smallest: the rival's smallest reachable city "
           "(nearest breaks ties); nearest: its nearest city.", ["smallest", "nearest"]),
        _n("target_pop_weight", 0.5, "Target: population weight", "Target score = distance + size x this - bonus "
           "if damaged; lowest wins.", 0, 5),
        _n("target_damaged_bonus", 6, "Target: damaged city bonus", "", 0, 30),
        _n("gather_radius", 4, "Gathered: within", "Units within this of the rally point or target count as "
           "gathered.", 0, 10, "tiles"),
        _n("advance_min", 3, "Advance: minimum units", "Advance with clamp(field army x share, min, max) gathered.",
           0, 20),
        _n("advance_max", 8, "Advance: units enough", "", 0, 30),
        _n("advance_share", 0.6, "Advance: share of field army", "", 0, 1),
        _n("advance_hold_min", 2, "Fall back below", "Return to the rally point with fewer than this near the "
           "target.", 0, 10),
        _n("weak_city_health", 0.5, "Weak target: health", "A target at or below this share of health is weak ...",
           0, 1),
        _n("weak_power_ratio", 3, "Weak target: power", "... or when its owner's power x this is below ours.", 1, 20,
           "x"),
        _n("weak_min_units", 2, "Weak target: units", "Advance on a weak target with this many field units.", 0, 10),
        _n("siege_ready_dist", 2, "Siege ready within", "Melee close in once a ranged unit is this close ...", 0, 5,
           "tiles"),
        _n("siege_ready_health", 0.5, "... or city health below", "", 0, 1),
        _n("rally_radius", 2, "At the rally point within", "", 0, 10, "tiles"),
        _n("rally_min_dist", 4, "Rally point: min distance", "From the target.", 1, 10, "tiles"),
        _n("rally_max_dist", 5, "Rally point: max distance", "", 1, 15, "tiles"),
        _n("rally_cover_bonus", 2, "Rally point: cover bonus", "Hills and woods count as this many tiles closer.",
           0, 10),
        _n("melee_standoff", 2, "Melee stand-off", "Melee units wait this far from the city until the siege is "
           "ready.", 0, 5, "tiles"),
    ]),
    ("Peace and diplomacy", "Peace offers, embassies, friendships, research agreements, luxury trades.", [
        _n("war_long_turns", 25, "War is long after", "The bot then looks for peace.", 1, 200, "turns"),
        _n("peace_min_turns", 10, "No peace offers before", "", 0, 100, "turns"),
        _n("peace_offer_chance", 0.35, "Peace offer chance", "Per turn, when losing or in a long war.", 0, 1),
        _n("losing_ratio", 0.9, "Losing below power ratio", "", 0, 5, "x"),
        _n("siege_progress_units", 2, "Siege progressing: units", "A siege with this many units within ...", 0, 10),
        _n("siege_progress_radius", 3, "... tiles", "", 0, 10, "tiles"),
        _n("siege_progress_health", 0.8, "... and city health below", "... is progressing: no peace offer.", 0, 1),
        _n("diplo_every", 10, "Diplomacy every", "Turns between embassy/friendship/agreement offers to a civ.", 1, 50,
           "turns"),
        _n("friend_max_ratio", 2, "Friendship: not if stronger", "No friendship offers when our power is this many "
           "times theirs.", 0, 20, "x"),
        _n("friend_chance", 0.4, "Friendship chance", "Chance = this - aggr x aggression.", 0, 1),
        _n("friend_chance_aggr", 0.3, "Friendship chance per aggression", "", 0, 1),
        _n("ra_gold_margin", 100, "Research agreement reserve", "Gold kept beyond the agreement's cost.", 0, 2000,
           "gold"),
        _n("lux_trade_every", 3, "Luxury trade every", "", 1, 50, "turns"),
        _n("lux_spare_at", 2, "Spare luxury at", "A luxury is spare with this many copies.", 1, 10),
        _n("lux_sell_gpt", 3, "Sell a luxury for", "Gold per turn asked when they have no luxury to swap.", 0, 50,
           "gold"),
        _n("lux_buyer_min_gold", 20, "Buyer needs gold", "", 0, 500, "gold"),
        _b("lux_buy", False, "Buy luxuries", "While happiness is short, offer gold per turn for a luxury type we lack "
           "to a civ with a spare (bots otherwise only offer their own surplus)."),
        _n("lux_buy_gpt", 2, "Pay for a luxury", "Gold per turn offered for a luxury we lack.", 1, 20, "gold"),
        _n("lux_buy_hap_margin", 2, "Buy luxuries below happiness", "Happiness is short below cities + this.",
           -10, 30),
    ]),
    ("Deals", "What deal items are worth, in gold. The same values judge offers made and received.", [
        _n("deal_gpt", 0.8, "Gold per turn", "Value per gold per turn per turn of the deal.", 0, 2),
        _n("deal_lux_needed", 60, "Luxury we need", "", 0, 500, "gold"),
        _n("deal_lux_spare", 15, "Spare luxury", "", 0, 500, "gold"),
        _n("deal_strategic_needed", 45, "Strategic we lack: base", "... + per era.", 0, 500, "gold"),
        _n("deal_strategic_needed_per_era", 15, "Strategic we lack: per era", "", 0, 200, "gold"),
        _n("deal_strategic_spare_per_era", 12, "Spare strategic: per era", "", 0, 200, "gold"),
        _n("deal_open_borders", 12, "Open borders", "", 0, 500, "gold"),
        _n("deal_embassy", 8, "Embassy", "", 0, 500, "gold"),
        _n("deal_peace", 120, "Peace", "Value of peace when not clearly stronger.", 0, 1000, "gold"),
        _n("deal_peace_strong_ratio", 1.3, "Clearly stronger above", "", 0, 5, "x"),
        _n("deal_peace_when_strong", 60, "Peace cost when stronger", "", 0, 1000, "gold"),
        _n("deal_peace_while_winning", 150, "Peace cost mid-campaign", "Cost of peace while our campaign against "
           "them is young and we are not weaker.", 0, 1000, "gold"),
        _n("deal_peace_campaign_turns", 40, "Campaign is young for", "", 0, 200, "turns"),
        _n("deal_peace_campaign_ratio", 0.8, "Not weaker above", "", 0, 5, "x"),
        _n("deal_friendship", 25, "Friendship or pact", "", 0, 500, "gold"),
        _n("deal_friendship_refuse", 80, "Refuse friendship", "Cost when we dislike them or plan war.", 0, 1000,
           "gold"),
        _n("deal_ra", 40, "Research agreement", "", 0, 500, "gold"),
        _n("deal_ra_margin", 50, "Research agreement reserve", "", 0, 1000, "gold"),
        _n("deal_ra_refuse", 100, "Refuse research agreement", "When we can't afford it.", 0, 1000, "gold"),
        _n("deal_map_get", 25, "Their map", "", 0, 500, "gold"),
        _n("deal_map_give", 20, "Our map", "", 0, 500, "gold"),
        _n("deal_city", 300, "City: base", "", 0, 5000, "gold"),
        _n("deal_city_per_pop", 60, "City: per citizen", "", 0, 1000, "gold"),
        _n("deal_tech_get", 0.6, "Tech received", "Share of its research cost.", 0, 3),
        _n("deal_tech_give", 0.9, "Tech given", "", 0, 3),
        _n("deal_ally_war", 200, "They join our war", "Them declaring on someone we fight or plan to fight.", 0,
           2000, "gold"),
        _n("deal_other_war", 20, "They declare on someone else", "", 0, 2000, "gold"),
        _n("deal_war_cost", 250, "Us declaring: base cost", "Cost = base + risk x max(0, their power / ours - "
           "from) ...", 0, 2000, "gold"),
        _n("deal_war_risk", 250, "Us declaring: risk", "", 0, 2000, "gold"),
        _n("deal_war_risk_from", 0.5, "Us declaring: risk from ratio", "", 0, 5),
        _n("deal_war_city_state", 300, "Declaring on a city-state", "", 0, 2000, "gold"),
        _n("deal_war_betrayal", 400, "Betraying a friend", "", 0, 2000, "gold"),
        _n("deal_war_planned_share", 0.3, "Already planned share", "Share of the cost when we meant to fight them "
           "anyway.", 0, 1),
        _n("deal_war_mult", 1.3, "War cost multiplier", "... times (this - aggr x aggression).", 0, 5, "x"),
        _n("deal_war_mult_aggr", 0.6, "War cost per aggression", "", 0, 5),
        _n("counter_rounds", 4, "Counter-offers up to", "Rounds of negotiation in which the bot still counters.",
           0, 20),
        _n("counter_max_gap", 150, "Counter within", "Counter (ask for gold) when a deal is short by less than this.",
           0, 2000, "gold"),
        _n("counter_margin", 10, "Counter margin", "Gold added on top of the shortfall.", 0, 500, "gold"),
    ]),
    ("Gold", "Reserves, purchases, deficits.", [
        _n("gold_reserve", 60, "Reserve", "Gold kept back = this + per era x era.", 0, 2000, "gold"),
        _n("gold_reserve_per_era", 25, "Reserve per era", "", 0, 500, "gold"),
        _n("buy_cap", 60, "Purchase cap", "Most spent on one item = max(this + per era x era (+ settler), spare x "
           "share).", 0, 2000, "gold"),
        _n("buy_cap_per_era", 60, "Purchase cap per era", "", 0, 500, "gold"),
        _n("buy_cap_settler", 40, "Purchase cap: settler extra", "", 0, 500, "gold"),
        _n("buy_cap_spare_share", 0.6, "Purchase cap: share of spare", "", 0, 1),
        _n("deficit_horizon", 8, "Deficit horizon", "Act on negative income when gold would run out within this.",
           0, 100, "turns"),
        _n("gold_focus_deficit", 3, "Gold focus below", "Put the biggest city on gold focus below -this per turn.",
           0, 100, "gold"),
        _n("gold_focus_release", 8, "Release gold focus above", "", -50, 100, "gold"),
    ]),
    ("City-states", "Gold gifts to city-states.", [
        _c("cs_gift_mode", "classic", "Gift mode", "classic: gift the most-liked city-state when rich; typed: gift "
           "the type we need where a gift crosses a threshold.", ["classic", "typed"]),
        _n("cs_gift_gold", 350, "Classic: gift above", "Gift when gold exceeds this + per era x era.", 0, 5000, "gold"),
        _n("cs_gift_gold_per_era", 80, "Classic: per era", "", 0, 1000, "gold"),
        _n("cs_gift_focus_influence", 30, "Classic: prefer influence above", "", 0, 200),
        _n("cs_gift_amount", 250, "Classic: gift", "", 0, 2000, "gold"),
        _n("cs_typed_reserve", 200, "Typed: reserve", "Gold kept = this + per era x era ...", 0, 5000, "gold"),
        _n("cs_typed_reserve_per_era", 60, "Typed: reserve per era", "", 0, 1000, "gold"),
        _n("cs_typed_min_spare", 150, "Typed: spare needed", "... plus at least this to spend.", 0, 2000, "gold"),
        _n("cs_typed_amount", 250, "Typed: gift", "", 0, 2000, "gold"),
        _n("cs_typed_big_above", 500, "Typed: big gift above", "Spare gold above which the big gift is used.", 0,
           5000, "gold"),
        _n("cs_typed_big_amount", 500, "Typed: big gift", "", 0, 5000, "gold"),
        _n("cs_typed_hap_margin", 2, "Typed: need happiness below", "Cities + this.", -10, 20),
        _n("cs_w_mercantile_unhappy", 3.0, "Mercantile (need happiness)", "", 0, 10),
        _n("cs_w_mercantile", 1.2, "Mercantile", "", 0, 10),
        _n("cs_w_maritime", 2.0, "Maritime", "", 0, 10),
        _n("cs_w_cultured", 1.5, "Cultured", "", 0, 10),
        _n("cs_w_religious", 0.8, "Religious", "", 0, 10),
        _n("cs_w_militaristic", 0.6, "Militaristic", "", 0, 10),
        _n("cs_v_friend", 1.0, "Becoming friends", "", 0, 10),
        _n("cs_v_ally", 2.0, "Becoming allies", "", 0, 10),
        _n("cs_v_defend", 1.5, "Keeping an alliance", "", 0, 10),
        _n("cs_typed_defend_margin", 15, "Alliance at risk within", "A rival within this influence of ours.", 0, 100),
        _n("cs_typed_safe_lead", 30, "Safe alliance lead", "No more gifts with this lead over every rival ...", 0,
           200),
        _n("cs_typed_safe_above_ally", 20, "... and this above the ally threshold", "", 0, 200),
    ]),
    ("Religion", "Beliefs and faith spending.", [
        _c("belief_mode", "prefs", "Belief choice", "prefs: the ranked lists below; unciv: UnCiv's AI weights.",
           ["prefs", "unciv"]),
        _o("beliefs_pantheon", BELIEF_PREFS["Pantheon"], "Pantheon beliefs", "In order of preference.",
           BELIEF_PREFS["Pantheon"] + [b for b in PANTHEON_PREFS if b not in BELIEF_PREFS["Pantheon"]]),
        _o("beliefs_founder", BELIEF_PREFS["Founder"], "Founder beliefs", "", BELIEF_PREFS["Founder"]),
        _o("beliefs_follower", BELIEF_PREFS["Follower"], "Follower beliefs", "", BELIEF_PREFS["Follower"]),
        _o("beliefs_enhancer", BELIEF_PREFS["Enhancer"], "Enhancer beliefs", "", BELIEF_PREFS["Enhancer"]),
        _o("pantheon_unciv", PANTHEON_PREFS, "Pantheon (unciv mode)", "Pantheon order when belief choice is unciv.",
           PANTHEON_PREFS + [b for b in BELIEF_PREFS["Pantheon"] if b not in PANTHEON_PREFS]),
        _b("faith_buildings", True, "Buy faith buildings", "Buy faith-only buildings (Pagoda, Mosque, Cathedral) "
           "before missionaries."),
        _n("faith_min", 200, "Spend faith above", "", 0, 5000, "faith"),
        _n("faith_building_keep", 50, "Faith kept after a building", "", 0, 2000, "faith"),
        _n("faith_missionary_keep", 100, "Faith kept after a missionary", "", 0, 2000, "faith"),
        _n("max_missionaries", 2, "Missionaries at once", "", 0, 20),
    ]),
    ("Policies and great people", "", [
        _o("policy_order_peaceful", "rationalism_early", "Policy order (peaceful)", "Branch order below the "
           "aggressive threshold.", POLICY_BRANCHES,
           presets={"default": POLICY_ORDER["peaceful"], **{k: v for k, v in POLICY_ORDERS.items() if v}}),
        _o("policy_order_aggressive", None, "Policy order (aggressive)", "", POLICY_BRANCHES,
           presets={"default": POLICY_ORDER["aggressive"], **{k: v for k, v in POLICY_ORDERS.items() if v}}),
        _n("policy_picks_per_turn", 4, "Policies per turn", "Most policies adopted in one turn.", 1, 10),
        _c("free_gp_early", "Great Scientist", "Free great person (early)", "", GREAT_PEOPLE),
        _c("free_gp_late", "Great Engineer", "Free great person (late)", "", GREAT_PEOPLE),
        _n("free_gp_switch_era", 4, "Late from era", "", 0, 8),
    ]),
]
PARAM_SPECS: list[dict] = [dict(s, group=name) for name, _, specs in PARAM_GROUPS for s in specs]
DEFAULT_PARAMS: dict = {s["key"]: s["default"] for s in PARAM_SPECS}


def cm_tiles(g, c) -> list:
    """The tiles a city owns."""
    from ..engine.cities import city_tiles
    return city_tiles(g, c)


def _ud(g, u) -> dict:
    """A unit's ruleset definition."""
    return g.rules.units[u.type]


def _is_recon(ud) -> bool:
    """Whether a unit type is a scout."""
    return ud.get("unitType") == "Scout"


def _can_embark(g, pid) -> bool:
    """Whether this civilization's land units can cross water."""
    from ..engine.movement import civ_can_embark
    return civ_can_embark(g, pid)


def _power(ud) -> float:
    """A unit type's combat weight, the larger of its melee and ranged strength."""
    return max(ud.get("strength", 0), ud.get("rangedStrength", 0))


class BasicBot:
    """The scripted opponent, and the yardstick every model score is measured against.

    A few thousand lines of heuristics with no learning in them, which is the point: it can be read,
    argued with and changed deliberately, and a benchmark denominated in it means something you can
    inspect.

    It plays the whole game - research by need, buildings chosen by simulating the city with each
    candidate, policies, religion, great people, espionage, city-states, war with siege units and rally
    points. ``aggression`` scales army size and willingness to start a war.

    Every number it uses lives in ``PARAM_GROUPS`` / ``DEFAULT_PARAMS`` and can be overridden per seat, which is what
    bot profiles and the lab's experiments vary. Its known ceiling is happiness: it stops expanding at two to
    five cities on a small map, and raising that is the open balance work.
    """
    def __init__(self, aggression: float = 0.4, seed: Optional[int] = None, target_cities: int = 0,
                 params: Optional[dict] = None):
        self.aggression = max(0.0, min(1.0, aggression))
        self.p = dict(DEFAULT_PARAMS)
        self.p.update(params or {})
        self.rng = random.Random(seed)
        self.target_cities = target_cities or self.p["target_cities"]     # 0 = expand while there is room
        self._sites_cache: dict = {}
        self._bad_sites: dict = {}                  # (pid, idx) -> turn a settler failed to reach it
        self._boat_turn: dict = {}                  # city id -> turn a work boat was last queued
        self._war_plan: dict = {}                   # pid -> {"city": idx, "rally": idx, "advance": bool, ...}
        self._war_prep: dict = {}                   # pid -> {"player": q, "since": turn}
        self._garrisons: dict = {}
        self._escorts: dict = {}                    # settler id -> escorting military unit id
        self._retreats: dict = {}                   # (pid, site) -> times a settler turned back from it
        self._need_escort: dict = {}                # pid -> city idx where a settler waits for an escort
        self._bv_cache: dict = {}                   # (city id, building) -> (turn, signature, value)
        self._space_res: Optional[set] = None       # resources spaceship parts need (from the ruleset, once)

    # ------------------------------------------------------------------
    def ex(self, g: Game, pid: int, _tool: str, **args):
        """Call a game tool, swallowing refusals.

        The bot proposes constantly and is refused often - a unit that cannot reach, a building that is no
        longer available - and treating every refusal as an error would mean checking each precondition
        twice. What it must never do is crash a game, so failures are counted and ignored.
        """
        try:
            return tools.execute(g, pid, _tool, args)
        except ActionError:
            return None

    def play_turn(self, g: Game, pid: int, end_turn: bool = True):
        """Play one complete turn."""
        if g.s.phase != "playing" or g.s.current != pid:
            return
        self.handle_negotiations(g, pid)
        ctx = self.context(g, pid)
        self.choose_research(g, pid, ctx)
        self.empire_choices(g, pid, ctx)
        self.city_bombard(g, pid)
        self.manage_units(g, pid, ctx)
        self.city_bombard(g, pid)
        ctx = self.context(g, pid)
        self.manage_cities(g, pid, ctx)
        self.manage_gold(g, pid, ctx)
        self.consider_diplomacy(g, pid, ctx)
        if end_turn and g.s.current == pid and g.s.phase == "playing":
            tools.execute(g, pid, "end_turn", {})

    # ------------------------------------------------------------------
    # situation
    # ------------------------------------------------------------------
    def context(self, g: Game, pid: int) -> dict:
        """Gather everything the turn's decisions need, once.

        The bot consults the same facts repeatedly - happiness, threats, army size, expansion sites - and
        computing them per decision was measurably slower than computing them per turn.
        """
        from ..engine import economy, research, visibility
        P = self.p
        p = g.player(pid)
        cities = g.player_cities(pid)
        units = g.player_units(pid)
        vis = visibility.visible_tiles(g, pid)
        hostile = [u for u in g.s.units.values() if u.idx in vis and g.at_war(pid, u.owner) and _ud(g, u)["_military"]
                   and visibility.unit_visible_to(g, pid, u)]
        threat, near_enemies = {}, {}
        radius, near_d, near_w, far_w = P["threat_radius"], P["threat_near_dist"], P["threat_near_weight"], \
            P["threat_far_weight"]
        for c in cities:
            t, near = 0.0, []
            for u in hostile:
                d = g.grid.distance(u.idx, c.idx)
                if d <= radius:
                    t += _power(_ud(g, u)) * u.hp / 100 * (near_w if d <= near_d else far_w)
                    near.append(u)
            threat[c.id] = t
            near_enemies[c.id] = near
        military = [u for u in units if _ud(g, u)["_military"] and not _is_recon(_ud(g, u))]
        st = economy.civ_stats(g, pid)
        wars = [q for q in p.met if g.player(q).alive and g.at_war(pid, q) and g.player(q).kind == "major"]
        preparing = pid in self._war_prep
        n = len(cities)
        army_target = int(n * (P["army_per_city"] + P["army_per_city_aggr"] * self.aggression) + P["army_base"]
                          + (n * P["army_war_per_city"] + P["army_war_extra"] if wars or preparing else 0)
                          + P["army_per_threatened_city"] * sum(1 for c in cities if threat[c.id] > 0))
        supply = economy.unit_supply(g, pid)
        from ..engine import tiles as T
        hap = economy.happiness(g, pid)
        lux_owned = set(hap.get("luxury_types", []))
        pending_res = set()           # resources in our borders that lack their improvement
        for c in cities:
            for idx in cm_tiles(g, c):
                t = g.s.tiles[idx]
                if t.resource and T.resource_visible(g, pid, t.resource) \
                        and not T.resource_improved_by(g, t.resource, t.improvement):
                    pending_res.add(t.resource)
        exposed = None
        if P["garrison_mode"] == "exposed":
            r = P["garrison_exposed_radius"]
            others = [c.idx for c in g.s.cities.values() if c.owner != pid and g.player(c.owner).kind != "city_state"]
            camps = [cp["idx"] for cp in g.s.camps.values() if not cp.get("destroyed")]
            exposed = {c.id for c in cities if c.id == (p.capital if p.capital is not None else c.id) or threat[c.id] > 0
                       or any(g.grid.distance(c.idx, o) <= r for o in others)
                       or any(g.grid.distance(c.idx, o) <= r for o in camps)}
        return {"lux_owned": lux_owned, "pending_res": pending_res, "exposed": exposed,
                "cities": cities, "units": units, "military": military, "hostile": hostile, "threat": threat,
                "near_enemies": near_enemies, "gpt": st["gold"], "hap": hap["total"],
                "era": research.player_era(g, pid), "wars": wars, "gold": p.gold, "supply": supply,
                "army_target": army_target, "vis": vis, "offense": bool(wars) or preparing}

    def city_defense(self, g: Game, city) -> float:
        """How well defended a city is, for deciding whether it needs another unit."""
        from ..engine import combat, cities as cm
        s = combat.city_strength(g, city) * city.health / max(1, cm.max_health(g, city))
        for idx in g.grid.within(city.idx, 1):
            m = g.military_at(idx)
            if m and m.owner == city.owner:
                s += _power(_ud(g, m)) * m.hp / 100
        return s

    def _breaks_space_reserve(self, g: Game, pid: int, name: str, era: int) -> bool:
        """Whether building `name` would use a resource kept for spaceship parts (see space_reserve)."""
        P = self.p
        if not P["space_reserve"] or era < P["tech_space_era"] or not g.victory_enabled("Scientific"):
            return False
        R = g.rules
        d = R.units.get(name) or R.buildings.get(name) or {}
        res = d.get("requiredResource")
        if not res or (name in R.units and R.units[name]["_umap"].get(U.SpaceshipPart)):
            return False
        need = self._space_res
        if need is None:
            need = self._space_res = {u.get("requiredResource") for u in R.units.values()
                                      if u["_umap"].get(U.SpaceshipPart) and u.get("requiredResource")}
        if res not in need:
            return False
        from ..engine import economy
        return (economy.strategic_resources(g, pid).get(res) or {}).get("available", 0) - 1 < P["space_reserve"]

    def needs_garrison(self, city, ctx: dict) -> bool:
        """Whether a city should keep a unit in it (always, with garrison_mode "all")."""
        return ctx.get("exposed") is None or city.id in ctx["exposed"]

    def in_danger(self, g: Game, city, ctx: dict) -> bool:
        """Whether the enemies near a city outweigh its defence enough to drop everything for it."""
        return ctx["threat"].get(city.id, 0) > self.city_defense(g, city) * self.p["danger_ratio"]

    # ------------------------------------------------------------------
    # research
    # ------------------------------------------------------------------
    def choose_research(self, g: Game, pid: int, ctx: Optional[dict] = None):
        """Pick what to research, beelining whatever is most valuable now."""
        from ..engine import research
        P = self.p
        p = g.player(pid)
        if p.free_techs > 0:
            avail = research.available_techs(g, pid)
            if avail:
                self.ex(g, pid, "choose_free_tech",
                        tech=max(avail, key=lambda t: research.tech_cost(g, pid, t)))
        if research.current(g, pid):
            return
        ctx = ctx or self.context(g, pid)
        avail = research.available_techs(g, pid)
        if not avail:
            return
        best, best_v = None, -1.0
        memo: dict = {}
        lo, hi = 1 - P["tech_noise"], 1 + P["tech_noise"]
        for t in g.rules.techs:
            if g.has_tech(pid, t) or research.is_unresearchable(g, pid, t):
                continue
            v = self._tech_value(g, pid, t, ctx, memo)
            if v <= 0:
                continue
            path = research.path_to(g, pid, t)
            if not path:
                continue
            cost = sum(research.tech_cost(g, pid, x) for x in path)
            v += P["tech_path_value"] * sum(self._tech_value(g, pid, x, ctx, memo) for x in path[:-1])
            score = v / (cost ** P["tech_cost_exp"]) * self.rng.uniform(lo, hi)
            if score > best_v:
                best, best_v = path[0], score
        self.ex(g, pid, "set_research", tech=best or min(avail, key=lambda t: research.tech_cost(g, pid, t)))

    def _tech_value(self, g: Game, pid: int, tech: str, ctx: dict, memo: Optional[dict] = None) -> float:
        """The value of a technology, memoised across one decision."""
        memo = {} if memo is None else memo
        if tech in memo:
            return memo[tech]
        v = memo[tech] = self._tech_value_uncached(g, pid, tech, ctx, memo)
        return v

    def _building_potential(self, g: Game, pid: int, b: str, ctx: dict, memo: dict) -> float:
        """Rough empire-wide value of having building b available: its flat stats, its percentage bonuses applied to
        the empire's current yields, and per-population rules, in every city (or once for a wonder)."""
        import re
        R, P = g.rules, self.p
        bd = R.buildings[b]
        key = "__empire"
        emp = memo.get(key)
        if emp is None:
            from ..engine import economy
            st = economy.civ_stats(g, pid)
            cities = ctx["cities"] or []
            emp = memo[key] = {"n": max(1, len(cities)), "pop": sum(c.pop for c in cities) / max(1, len(cities)),
                               **{k: max(0.0, st.get(k, 0)) for k in ("food", "production", "gold", "science", "culture", "faith")}}
        w = {k: P["u_" + k] for k in ("food", "production", "gold", "science", "culture", "faith", "happiness")}
        per_city = sum(bd.get(k, 0) * w[k] for k in w)
        per_city -= bd.get("maintenance", 0) * w["gold"]
        for k, pct in (bd.get("percentStatBonus") or {}).items():
            if k in w:
                per_city += pct / 100 * emp.get(k, 0) / emp["n"] * w[k]
        for text in bd.get("uniques", []):
            m = re.match(r"\[\+(\d+) (\w+)\] per \[(\d+)\] population", text)
            if m and m.group(2).lower() in w:
                per_city += int(m.group(1)) * emp["pop"] / int(m.group(3)) * w[m.group(2).lower()]
        per_city += sum((bd.get("specialistSlots") or {}).values()) * P["pot_specialist"]
        mult = 1 if (bd.get("isWonder") or bd.get("isNationalWonder")) else emp["n"]
        return P["pot_base"] + max(0.0, per_city) * mult

    def _tech_value_uncached(self, g: Game, pid: int, tech: str, ctx: dict, memo: dict) -> float:
        """Score a technology by what it unlocks and what the civilization currently needs.

        Includes luxuries it cannot yet improve, which is deliberate: the happiness ceiling makes the
        technology that lifts it worth more than its immediate yield suggests.
        """
        R, P = g.rules, self.p
        un = R.unlocks.get(tech, {})
        v = P["tech_base_value"]
        for b in un.get("buildings", []):
            bd = R.buildings[b]
            if bd.get("uniqueTo") and bd["uniqueTo"] != g.player(pid).nation:
                continue
            if P["tech_mode"] == "potential":
                v += self._building_potential(g, pid, b, ctx, memo)
                continue
            v += P["tech_building_base"] + sum(bd.get(k, 0) * P["tw_" + k] for k in STAT_KEYS) \
                + P["tech_building_unique"] * len(bd.get("uniques", []))
            if bd.get("isWonder"):
                v += P["tech_wonder"]
        best_now = memo.get("__best_now")
        if best_now is None:
            best_now = memo["__best_now"] = max(
                (_power(ud) for n, ud in R.units.items() if ud["_military"] and ud["_domain"] == "Land"
                 and g.has_tech(pid, ud.get("requiredTech"))), default=P["tech_default_power"])
        for n in un.get("units", []):
            ud = R.units[n]
            if ud.get("uniqueTo") and ud["uniqueTo"] != g.player(pid).nation:
                continue
            if not ud["_military"]:
                v += P["tech_civilian_unit"]
                continue
            gain = _power(ud) / max(1, best_now)
            if gain > P["tech_unit_min_gain"]:
                v += (P["tech_unit_base"] + P["tech_unit_gain"] * (gain - 1)) \
                    * (P["tech_unit_war_mult"] if ctx["wars"] else 1.0) * (P["tech_unit_aggr_base"] + self.aggression)
        v += P["tech_improvement"] * len(un.get("improvements", [])) + P["tech_reveal"] * len(un.get("reveals", []))
        from ..engine import tiles as T
        for res in ctx.get("pending_res", ()):
            rt = R.resources[res]["resourceType"]
            if any(T.resource_improved_by(g, res, imp) for imp in un.get("improvements", [])):
                if rt == "Luxury" and res not in ctx["lux_owned"]:
                    v += P["tech_lux_needed"] if ctx["hap"] < len(ctx["cities"]) + P["tech_lux_hap_margin"] \
                        else P["tech_lux"]
                elif rt == "Strategic":
                    v += P["tech_strategic"]
        v += P["tech_unique"] * len(R.techs[tech].get("uniques", []))
        if ctx["era"] >= P["tech_space_era"] and any(R.units[n]["_umap"].get(U.SpaceshipPart)
                                                     or R.units[n]["_umap"].get(U.AddInCapital)
                                                     for n in un.get("units", [])):
            v += P["tech_space"]
        return v

    # ------------------------------------------------------------------
    # empire-wide choices: policies, religion, great people, spies
    # ------------------------------------------------------------------
    def _policy_order(self) -> list:
        """The policy branch order for this bot's temperament: a named preset, a custom list, or the default."""
        aggressive = self.aggression > self.p["aggressive_above"]
        order = self.p.get("policy_order_aggressive" if aggressive else "policy_order_peaceful")
        if isinstance(order, str):
            order = POLICY_ORDERS.get(order)
        return order or POLICY_ORDER["aggressive" if aggressive else "peaceful"]

    def empire_choices(self, g: Game, pid: int, ctx: dict):
        """Spend accumulated culture and faith: policies, beliefs, great people."""
        from ..engine import policies, religion
        P = self.p
        p = g.player(pid)
        order = self._policy_order()
        for _ in range(P["policy_picks_per_turn"]):
            if not policies.can_adopt_any(g, pid):
                break
            options = policies.adoptable_policies(g, pid)
            if not options:
                break

            def rank(name):
                """Score one policy or branch."""
                br = policies.branch_of(g, name) if not policies.is_branch(g, name) else name
                base = order.index(br) if br in order else 50
                return (0 if br in p.policies else 1, base, name)   # finish open branches first
            if self.ex(g, pid, "adopt_policy", policy=min(options, key=rank)) is None:
                break
        if p.free_great_people > 0:
            gp = P["free_gp_early"] if ctx["era"] < P["free_gp_switch_era"] else P["free_gp_late"]
            self.ex(g, pid, "choose_great_person", great_person=gp)
        if g.religion_enabled and religion.can_found_pantheon(g, pid) is None:
            avail = religion.beliefs_available(g, "Pantheon")
            prefs = P["beliefs_pantheon"] if P["belief_mode"] == "prefs" else P["pantheon_unciv"]
            pick = next((b for b in prefs if b in avail), avail[0] if avail else None)
            if pick:
                self.ex(g, pid, "found_pantheon", belief=pick)
        if g.espionage_enabled:
            self._spies(g, pid, ctx)

    def _spies(self, g: Game, pid: int, ctx: dict):
        """Send spies somewhere useful, and stage a coup when it looks winnable."""
        p = g.player(pid)
        taken = {s.get("city") for s in p.spies}
        for s in p.spies:
            if s["action"] != "None":
                continue
            targets = []
            for q in g.majors():
                if q.id == pid or not g.has_met(pid, q.id):
                    continue
                cap = g.city(q.capital) if q.capital is not None else None
                if cap and p.explored[cap.idx] and cap.id not in taken:
                    lead = len(q.techs) - len(p.techs)
                    targets.append((lead + self.rng.random(), cap.id))
            if targets:
                _, cid = max(targets)
            else:
                cap = g.city(p.capital) if p.capital is not None else None
                cid = cap.id if cap is not None and cap.id not in taken else None
            if cid is not None and self.ex(g, pid, "move_spy", spy=s["name"], city_id=cid) is not None:
                taken.add(cid)

    # ------------------------------------------------------------------
    # cities
    # ------------------------------------------------------------------
    def expansion_sites(self, g: Game, pid: int, ctx: dict) -> list[int]:
        """Where this civilization should put its next cities, best first."""
        from ..engine.automation import city_site_score
        from ..engine import tiles as T
        from ..engine import cities as cm
        P = self.p
        cached = self._sites_cache.get(pid)
        if cached and g.turn - cached[0] < P["site_cache_turns"]:
            return [i for i in cached[1] if cm.found_check(g, pid, i) is None]
        cities = ctx["cities"]
        centers = [c.idx for c in cities] or [u.idx for u in ctx["units"] if _ud(g, u)["_umap"].get(U.FoundCity)]
        cont = g.s.continents
        reachable = {cont[c] for c in centers}
        seen, scored = set(), []
        for center in centers:
            for idx in g.grid.within(center, P["site_radius"]):
                if idx in seen:
                    continue
                seen.add(idx)
                if g.is_water(idx) or g.grid.distance(center, idx) < P["site_min_distance"] or cont[idx] not in reachable:
                    continue
                if g.turn - self._bad_sites.get((pid, idx), -99) < P["site_blacklist_turns"]:
                    continue
                s = city_site_score(g, pid, idx)
                if s is None or s < P["site_min_score"]:
                    continue
                if P["site_new_lux"]:
                    # a luxury type we don't have yet is +4 happiness empire-wide: the resource expansion needs
                    new = {g.s.tiles[n].resource for n in g.grid.within(idx, P["site_lux_radius"]) if g.s.tiles[n].resource
                           and g.s.tiles[n].owner in (None, pid) and T.resource_visible(g, pid, g.s.tiles[n].resource)
                           and g.rules.resources[g.s.tiles[n].resource]["resourceType"] == "Luxury"} - ctx.get("lux_owned", set())
                    s += P["site_new_lux"] * len(new)
                d = min(g.grid.distance(c, idx) for c in centers)
                scored.append((s - d * P["site_distance_cost"], idx))
        scored.sort(reverse=True)
        picked = []
        for s, idx in scored:
            if all(g.grid.distance(idx, o) >= P["site_spacing"] for o in picked):
                picked.append(idx)
            if len(picked) >= P["site_candidates"]:
                break
        self._sites_cache[pid] = (g.turn, picked)
        return picked

    def _counts(self, g: Game, ctx: dict) -> tuple[dict, int]:
        """What the civilization already has, by unit kind, and its total army strength."""
        R = g.rules
        counts = {"settler": 0, "worker": 0, "recon": 0, "boat": 0}

        def kind(name):
            """Classify a unit type for counting."""
            ud = R.units.get(name)
            if not ud:
                return None
            if ud["_umap"].get(U.FoundCity):
                return "settler"
            if ud["_umap"].get(U.BuildImprovements):
                return "worker"
            if ud["_umap"].get(U.CreateWaterImprovements):
                return "boat"
            if _is_recon(ud):
                return "recon"
            return "army" if ud["_military"] else None
        army = len(ctx["military"])
        for u in ctx["units"]:
            k = kind(u.type)
            if k in counts:
                counts[k] += 1
        for c in ctx["cities"]:
            for q in c.queue:
                k = kind(q)
                if k in counts:
                    counts[k] += 1
                elif k == "army":
                    army += 1
        return counts, army

    def advise_production(self, g: Game, pid: int, city) -> Optional[str]:
        """What this bot would build next in one city (used for human players' 'auto production' cities)."""
        ctx = self.context(g, pid)
        counts, army = self._counts(g, ctx)
        return self._choose_production(g, pid, city, ctx, counts, army, self.in_danger(g, city, ctx))

    def manage_cities(self, g: Game, pid: int, ctx: Optional[dict] = None):
        """Run every city: citizens, production and purchases."""
        ctx = ctx or self.context(g, pid)
        counts, army = self._counts(g, ctx)
        for c in sorted(ctx["cities"], key=lambda c: -ctx["threat"][c.id]):
            if c.puppet:
                continue
            danger = self.in_danger(g, c, ctx)
            if c.queue:
                head = c.queue[0]
                is_military = head in g.rules.units and g.rules.units[head]["_military"]
                if not (danger and not is_military):
                    continue
            choice = self._choose_production(g, pid, c, ctx, counts, army, danger)
            if choice and self.ex(g, pid, "set_production", city_id=c.id, item=choice) is not None:
                ud = g.rules.units.get(choice)
                if ud:
                    if ud["_umap"].get(U.FoundCity):
                        counts["settler"] += 1
                    elif ud["_umap"].get(U.BuildImprovements):
                        counts["worker"] += 1
                    elif ud["_umap"].get(U.CreateWaterImprovements):
                        counts["boat"] += 1
                        self._boat_turn[c.id] = g.turn
                    elif _is_recon(ud):
                        counts["recon"] += 1
                    elif ud["_military"]:
                        army += 1
            self._manage_city_tiles(g, pid, c, ctx)

    def _manage_city_tiles(self, g: Game, pid: int, c, ctx: dict):
        """Set a city's focus and citizen assignment for its situation."""
        from ..engine import cities as cm
        P = self.p
        if c.razing or c.puppet:
            return
        if P["unhappy_avoid_growth"]:
            # while unhappy (-75% growth everywhere), stop the biggest cities from growing so the rest grow normally
            big = sorted(ctx["cities"], key=lambda x: -x.pop)[:max(1, len(ctx["cities"]) // P["avoid_growth_div"])]
            stop = ctx["hap"] < 0 and c in big and c.pop >= P["avoid_growth_min_pop"]
            if stop != bool(c.avoid_growth) and (stop or ctx["hap"] >= P["avoid_growth_release"]):
                self.ex(g, pid, "set_city_focus", city_id=c.id, avoid_growth=stop)
        want = "balanced"
        small = P["small_city_focus"]
        small = None if small == "none" else small
        if small and c.pop < P["small_city_pop"] and not cm.is_capital(g, c):
            want = small                     # small cities grow first
        if c.focus == "gold":
            return                           # set and cleared by manage_gold during deficits
        if c.focus != want:
            self.ex(g, pid, "set_city_focus", city_id=c.id, focus=want)

    def _choose_production(self, g: Game, pid: int, c, ctx: dict, counts: dict, army: int, danger: bool) -> Optional[str]:
        """Choose what a city builds, by whichever production heuristic is configured."""
        if self.p.get("prod_mode") == "unciv":
            return self._choose_production_unciv(g, pid, c, ctx, counts, army, danger)
        return self._choose_production_classic(g, pid, c, ctx, counts, army, danger)

    def _may_build_settler(self, g: Game, c, ctx: dict, counts: dict, sites: list, check_size: bool = True) -> bool:
        """Whether this city may start a settler: a site in range, room under the city cap and the settler cap, a
        big enough city (UnCiv mode only) and enough happiness."""
        P, n = self.p, len(ctx["cities"])
        return any(g.grid.distance(s, c.idx) <= P["settler_site_range"] for s in sites) \
            and n + counts["settler"] < (self.target_cities or 99) \
            and counts["settler"] < max(1, n // P["settler_per_cities"]) \
            and (not check_size or c.pop >= P["settler_min_pop"]) \
            and (ctx["hap"] >= P["settler_min_hap"] or (ctx["hap"] >= P["settler_min_hap_small"]
                                                        and n < P["settler_small_empire"]))

    # ------------------------------------------------------------------
    # UnCiv-style production (ConstructionAutomation): every option gets a value ("choice modifier"); the city
    # builds the best value per point of remaining production. Emergencies, settlers and scouting are overrides.
    # ------------------------------------------------------------------
    def _choose_production_unciv(self, g: Game, pid: int, c, ctx: dict, counts: dict, army: int, danger: bool):
        """Choose production the way UnCiv's AI does: weighted preferences over everything buildable.

        The default, because it was measurably better than the older ranking - it considers the whole
        buildable set with modifiers rather than walking a priority list.
        """
        from ..engine import cities as cm
        from math import sqrt
        R, P = g.rules, self.p
        cities = ctx["cities"]
        n = len(cities)
        items = cm.buildable_items(g, c)
        units = set(items["units"])
        prods = {x.id: max(1.0, cm.city_stats(g, x)["total"]["production"]) for x in cities if not x.puppet}
        prod = prods.get(c.id, 1.0)
        over_avg = prod >= sum(prods.values()) / max(1, len(prods))
        total_pop = sum(x.pop for x in cities)
        defender = self.best_military(g, c, units, prefer_ranged=True)
        # --- overrides ---------------------------------------------------------------------------------
        if defender and danger:
            return defender
        if defender and g.military_at(c.idx) is None and (g.turn > P["garrison_after_turn"] or ctx["hostile"]) \
                and self.needs_garrison(c, ctx):
            return defender
        if defender and self._need_escort.get(pid) == c.idx:
            self._need_escort.pop(pid, None)
            return defender
        settler = next((u for u in units if R.units[u]["_umap"].get(U.FoundCity)), None)
        sites = self.expansion_sites(g, pid, ctx)
        choices: list[tuple[float, str]] = []

        def add(name, modifier):
            """Add a candidate with its weight."""
            if name and modifier > 0:
                choices.append((modifier / max(1.0, cm.remaining_work(g, c, name)), name))
        if settler and self._may_build_settler(g, c, ctx, counts, sites):
            add(settler, P["u_settler"])
        met = self._knows_rival_city(g, pid)
        scout = next((u for u in units if _is_recon(R.units[u])), None)
        if scout and counts["recon"] == 0 and not met and g.turn < P["scout_until_turn"]:
            add(scout, P["u_scout"])
        if not met and g.turn > P["seek_after_turn"] and counts["recon"] == 0 and army <= len(ctx["military"]) \
                and not any(x.activity == "explore" for x in ctx["units"]):
            add(defender if (g.turn > P["seek_military_after"] or not scout) else scout, P["u_seeker"])
        # --- workers (UnCiv addWorkerChoice) -----------------------------------------------------------
        worker = next((u for u in units if R.units[u]["_umap"].get(U.BuildImprovements)), None)
        if worker:
            wpc, full = P["workers_per_city"], P["workers_full_until"]
            want = 1 if n <= 1 else (wpc * n if n <= full else wpc * full + wpc * P["workers_extra_share"] * (n - full))
            unimproved = sum(1 for x in cities for i in x.worked if not g.s.tiles[i].improvement
                             and not g.is_water(i))
            want += P["worker_unimproved"] * unimproved
            if counts["worker"] < want:
                add(worker, P["u_worker"] * want / (counts["worker"] + P["worker_count_offset"]))
        # --- work boats -------------------------------------------------------------------------------------
        boat = next((u for u in units if R.units[u]["_umap"].get(U.CreateWaterImprovements)), None)
        if boat and counts["boat"] == 0:
            from ..engine import tiles as T
            if any(g.s.tiles[i].owner == pid and g.s.tiles[i].resource and g.is_water(i) and not g.s.tiles[i].improvement
                   and T.resource_visible(g, pid, g.s.tiles[i].resource)
                   for i in g.grid.within(c.idx, P["boat_search_radius"])):
                add(boat, P["u_boat"])
        # --- military (UnCiv addMilitaryUnitChoice, plus war preparation) ------------------------------
        at_war = bool(ctx["wars"])
        mil = len(ctx["military"])
        if defender and (at_war or ctx["offense"] or over_avg) and len(ctx["units"]) < ctx["supply"] \
                and (at_war or ctx["offense"] or (ctx["gpt"] >= 0 and mil <= max(P["peace_army_min"], n * P["peace_army_per_city"]))) \
                and ctx["gold"] > P["military_min_gold"]:
            modifier = P["mil_base"] + sqrt(n / (mil + 1)) * P["mil_scale"]
            if at_war:
                modifier *= P["mil_war_mult"]
            if ctx["offense"] and army < ctx["army_target"]:
                modifier *= P["u_offense"] * (P["mil_offense_aggr_base"] + self.aggression)
            if ctx["hostile"] and any(g.is_barbarian(e.owner) for e in ctx["hostile"]):
                modifier = max(modifier, P["mil_barbarian_min"])
            if not over_avg:
                modifier /= P["mil_below_avg_div"]
            if army >= ctx["army_target"] and not at_war:
                modifier /= P["mil_army_full_div"]
            unit = self._pick_military(g, c, ctx, units, mil, n) or defender
            add(unit, P["u_military"] * modifier)
        # --- buildings ------------------------------------------------------------------------------------
        wonders = items["wonders"] if (not c.puppet and (not P["u_wonder_gate"] or (over_avg and total_pop >= P["wonder_gate_pop"]))) else []
        for b in items["buildings"] + wonders:
            if not self._breaks_space_reserve(g, pid, b, ctx["era"]):
                add(b, self._building_value_unciv(g, pid, c, b, ctx, over_avg))
        for name in items.get("other", []):
            if name not in cm.PERPETUAL and R.buildings.get(name) is not None:
                add(name, P["u_project"] if over_avg else P["u_project_low"])     # projects: Apollo, Manhattan...
        for name in units:
            ud = R.units[name]
            if (ud["_umap"].get(U.AddInCapital) or ud["_umap"].get(U.SpaceshipPart)) and over_avg:
                add(name, P["u_spaceship"])
        if not choices:
            for perp in ("Science", "Gold"):
                if perp in items.get("other", []):
                    return perp
            return defender
        return max(choices)[1]

    def _pick_military(self, g: Game, c, ctx: dict, units: set, mil: int, n: int) -> Optional[str]:
        """Which military unit to build: melee first when the field army lacks city-takers, then siege when
        attacking with too few, else the best unit (sometimes preferring ranged)."""
        P = self.p
        siege = sum(1 for u in ctx["military"] if _ud(g, u).get("unitType") == "Siege")
        want_siege = ctx["offense"] and siege * P["siege_per_units"] < mil - n + 1
        # only melee units capture cities: keep some in the field army
        field_melee = sum(1 for m in ctx["military"] if not _ud(g, m)["_ranged"] and not self._is_garrison(m))
        want_melee = ctx["offense"] and field_melee < P["min_field_melee"]
        return (self.best_military(g, c, units, offense=True, melee_only=True) if want_melee else None) \
            or (self.best_military(g, c, units, siege_only=True) if want_siege else None) \
            or self.best_military(g, c, units, prefer_ranged=self.rng.random() < P["ranged_chance"], offense=ctx["offense"])

    def _building_value_unciv(self, g: Game, pid: int, c, b: str, ctx: dict, over_avg: bool) -> float:
        """Cached _building_value_unciv_raw: values change slowly, and simulating every candidate building in every
        city each time a queue empties was a large share of bot time."""
        k = self.p["bv_cache_turns"]
        if not k:
            return self._building_value_unciv_raw(g, pid, c, b, ctx, over_avg)
        sig = (c.pop, len(c.buildings), ctx["hap"] < 0, ctx["hap"] < len(ctx["cities"]), bool(ctx["wars"]), over_avg,
               ctx["threat"].get(c.id, 0) > 0)
        hit = self._bv_cache.get((c.id, b))
        if hit is not None and hit[1] == sig and g.turn - hit[0] < k:
            return hit[2]
        v = self._building_value_unciv_raw(g, pid, c, b, ctx, over_avg)
        self._bv_cache[(c.id, b)] = (g.turn, sig, v)
        return v

    def _building_value_unciv_raw(self, g: Game, pid: int, c, b: str, ctx: dict, over_avg: bool) -> float:
        """UnCiv getValueOfBuilding: weighted stat difference + military, victory and one-time bonuses."""
        from ..engine import cities as cm
        R, P = g.rules, self.p
        bd = R.buildings[b]
        base = cm.city_stats(g, c)["total"]
        base_hap = sum(cm.city_happiness(g, c).values())
        try:
            after, hap = self._simulate(g, c, b)
        except Exception:
            return 0.0
        d = {k: after.get(k, 0) - base.get(k, 0) for k in ("food", "production", "gold", "science", "culture", "faith")}
        gold_w = P["u_gold"] * (P["u_gold_broke_mult"] if ctx["gold"] < 0 and ctx["gpt"] <= 0 else 1)
        cult_w = P["u_culture"] * (P["u_culture_first_mult"] if base.get("culture", 0) < P["u_culture_first_below"] else 1)
        hap_w = P["u_happiness"] * (P["u_happiness_low"] if ctx["hap"] < P["u_happiness_low_below"]
                                    or ctx["hap"] < len(ctx["cities"]) else 1)
        v = (d["food"] * P["u_food"] + d["production"] * P["u_production"] + d["gold"] * gold_w
             + d["science"] * P["u_science"] + d["culture"] * cult_w + d["faith"] * P["u_faith"] + (hap - base_hap) * hap_w)
        # carry-over food (Aqueduct/Granary-style): the food it saves per turn
        for u in bd["_umap"].get(U.CarryOverFood) if hasattr(U, "CarryOverFood") else ():
            v += max(0.0, base.get("food", 0)) * u.n(0) / 100 * P["u_food"] * P["u_carryover_share"]
        v += sum(bd.get("greatPersonPoints", {}).values()) * P["u_gpp"]
        v += sum(bd.get("specialistSlots", {}).values()) * P["u_specialist"]
        war = P["u_defense_war"] if ctx["wars"] else P["u_defense_peace"]
        if ctx["threat"].get(c.id, 0) > 0:
            war *= P["u_defense_threat_mult"]
        if bd.get("cityHealth"):
            v += war * bd["cityHealth"] / max(1, cm.max_health(g, c)) * P["u_city_health"]
        if bd.get("cityStrength"):
            from ..engine import combat
            v += war * bd["cityStrength"] / (combat.city_strength(g, c) + P["u_city_strength_offset"]) * P["u_city_strength"]
        if bd.get("isWonder") or bd.get("isNationalWonder"):
            v += P["u_wonder_bonus"] if bd.get("isWonder") else P["u_wonder_bonus"] * P["u_national_wonder_share"]
        if over_avg and any(x.ph in ("Triggers a Cultural Victory upon completion", "Triggers victory")
                            for x in bd["_umap"].all):
            v += P["u_victory_building"]
        if P["u_space_program"] and over_avg and any(x.ph == "Enables construction of Spaceship parts"
                                                     for x in bd["_umap"].all):
            v += P["u_space_program"]
        return v

    def _choose_production_classic(self, g: Game, pid: int, c, ctx: dict, counts: dict, army: int, danger: bool) -> Optional[str]:
        """The older production heuristic: needs first, then value per turn.

        Kept because it is a useful control in lab experiments - a second opinion to measure the default
        against.
        """
        from ..engine import cities as cm
        R, P = g.rules, self.p
        options: list[tuple[float, str]] = []
        cities = ctx["cities"]
        n = len(cities)
        items = cm.buildable_items(g, c)
        units = set(items["units"])
        prod = max(1.0, cm.city_stats(g, c)["total"]["production"])

        def turns(name):
            """Turns for this city to finish an item."""
            return max(1.0, cm.remaining_work(g, c, name) / prod)

        defender = self.best_military(g, c, units, prefer_ranged=True)
        garrison = g.military_at(c.idx)
        if defender and self._need_escort.get(pid) == c.idx:     # a settler is waiting here for an escort
            self._need_escort.pop(pid, None)
            options.append((P["c_escort"], defender))
        if defender:
            if danger:
                options.append((P["c_danger"], defender))
            elif garrison is None and (g.turn > P["garrison_after_turn"] or ctx["hostile"]) and self.needs_garrison(c, ctx):
                options.append((P["c_garrison"], defender))
            affordable = len(ctx["units"]) < ctx["supply"] and ctx["gpt"] >= P["c_military_min_gpt"]
            target = ctx["army_target"]
            if army < target and (affordable or ctx["offense"]):
                attacker = self._pick_military(g, c, ctx, units, len(ctx["military"]), n) or defender
                shortfall = 1 - army / max(1, target)
                base = (P["c_army_offense"] + P["c_army_offense_aggr"] * self.aggression) if ctx["offense"] \
                    else (P["c_army_peace"] + P["c_army_peace_aggr"] * self.aggression)
                options.append((base * (P["c_army_short_base"] + P["c_army_short_scale"] * shortfall), attacker))
        settler = next((u for u in units if R.units[u]["_umap"].get(U.FoundCity)), None)
        sites = self.expansion_sites(g, pid, ctx)
        if settler and not danger and self._may_build_settler(g, c, ctx, counts, sites, check_size=False):
            options.append(((P["settler_prio"] if g.turn < P["settler_prio_until"] else P["settler_prio_late"])
                            - n * P["settler_prio_per_city"], settler))
        met_anyone = self._knows_rival_city(g, pid)        # a rival's city location is known (contact isn't enough)
        scout = next((u for u in units if _is_recon(R.units[u])), None)
        if scout and counts["recon"] == 0 and (not met_anyone or g.turn < P["c_scout_early_turns"]) \
                and g.turn < P["scout_until_turn"]:
            options.append((P["c_scout_needed"] if not met_anyone and g.turn > P["c_scout_after_turn"] else P["c_scout"],
                            scout))
        # still alone and nobody exploring (scouts often die to barbarians): send a sturdier unit to find the others
        if not met_anyone and g.turn > P["seek_after_turn"] and counts["recon"] == 0 and army <= len(ctx["military"]) \
                and not any(x.activity == "explore" for x in ctx["units"]):
            seeker = defender if (g.turn > P["seek_military_after"] or not scout) else scout
            if seeker:
                options.append((P["c_seeker"], seeker))
        worker = next((u for u in units if R.units[u]["_umap"].get(U.BuildImprovements)), None)
        if worker:
            want = n + (P["c_worker_extra"] if n <= P["c_worker_extra_until"] else 0)
            if counts["worker"] < want:
                options.append((P["c_worker_urgent"] if counts["worker"] < max(1, n // P["c_worker_urgent_div"])
                                else P["c_worker"], worker))
        boat = next((u for u in units if R.units[u]["_umap"].get(U.CreateWaterImprovements)), None)
        if boat and counts["boat"] == 0 and g.turn - self._boat_turn.get(c.id, -99) > P["c_boat_retry_turns"]:
            from ..engine import tiles as T
            for idx in g.grid.within(c.idx, P["boat_search_radius"]):
                t = g.s.tiles[idx]
                if t.owner == pid and t.resource and g.is_water(idx) and not t.improvement \
                        and T.resource_visible(g, pid, t.resource):
                    options.append((P["c_boat"], boat))
                    break
        wonders = items["wonders"]
        if wonders and (sum(x.pop for x in cities) < P["wonder_min_pop"] or (P["wonder_avg_prod"] and prod <
                        sum(max(1.0, cm.city_stats(g, x)["total"]["production"]) for x in cities) / max(1, n))):
            wonders = []
        for b in items["buildings"] + wonders:
            if self._breaks_space_reserve(g, pid, b, ctx["era"]):
                continue
            v = self._building_value(g, pid, c, b, ctx)
            if v <= 0:
                continue
            options.append((v * P["c_building_scale"] / (1 + turns(b) / P["c_building_turns"]), b))
        for name in items.get("other", []):
            if name in cm.PERPETUAL:
                continue
            bd = R.buildings.get(name)
            if bd is not None:
                options.append((P["c_project"], name))       # projects (Apollo, Manhattan, Utopia...)
        for name in units:
            ud = R.units[name]
            if ud["_umap"].get(U.AddInCapital) or ud["_umap"].get(U.SpaceshipPart):
                options.append((P["c_spaceship"], name))
        if not options:
            if "Gold" in items.get("other", []):
                return "Gold"
            return self.best_military(g, c, units)
        options.sort(key=lambda o: -o[0])
        return options[0][1]

    def _simulate(self, g: Game, c, add: str) -> tuple[dict, float]:
        """City stats and city happiness with `add` built (UnCiv getStatDifferenceFromBuilding): the building is
        added temporarily and the caches are restored afterwards."""
        from ..engine import cities as cm
        saved_y, saved_c = g._ycache, g._cache
        g._ycache, g._cache = {}, {}
        c.buildings.append(add)
        try:
            st = cm.city_stats(g, c)["total"]
            hap = sum(cm.city_happiness(g, c).values())
        finally:
            c.buildings.remove(add)
            g._ycache, g._cache = saved_y, saved_c
        return st, hap

    def _building_value(self, g: Game, pid: int, c, b: str, ctx: dict) -> float:
        """Score a building by simulating the city with it built.

        More expensive than a static priority list and much better: the value of a granary depends on the
        city, and simulating answers that directly instead of approximating it.
        """
        from ..engine import cities as cm
        R = g.rules
        bd = R.buildings[b]
        base = cm.city_stats(g, c)["total"]
        base_hap = sum(cm.city_happiness(g, c).values())
        try:
            after, hap = self._simulate(g, c, b)
        except Exception:
            return 0.0
        P = self.p
        diff = {k: after.get(k, 0) - base.get(k, 0) for k in STAT_KEYS if k != "happiness"}
        v = sum(diff[k] * P["w_" + k] for k in diff)
        dh = hap - base_hap
        want = len(ctx["cities"]) + P["w_hap_margin"]
        v += dh * P["w_happiness"] * (P["w_hap_unhappy"] if ctx["hap"] < 0 else (P["w_hap_low"] if ctx["hap"] < want
                                                                                 else P["w_hap_ok"]))
        if ctx["gpt"] < 0 and diff.get("gold", 0) > 0:
            v += diff["gold"] * P["w_gold_broke"]
        v += sum(bd.get("greatPersonPoints", {}).values()) * P["w_gpp"]
        v += sum(bd.get("specialistSlots", {}).values()) * P["w_specialist"]
        if bd.get("cityStrength") or bd.get("cityHealth"):
            border = ctx["wars"] or ctx["threat"][c.id] > 0
            v += (bd.get("cityStrength", 0) / P["w_def_strength_div"] + bd.get("cityHealth", 0) / P["w_def_health_div"]) \
                * (P["w_def_border"] if border else P["w_def_interior"])
        v += P["unique_bonus"] * len(bd.get("uniques", []))   # effects the simulation cannot see (free units, XP...)
        if bd.get("isWonder"):
            v = v * P["wonder_mult"] + P["wonder_bonus"]
        if c.pop <= P["w_small_city_pop"] and not bd.get("isWonder"):
            v *= P["w_small_city_mult"]
        return v

    def best_military(self, g: Game, city, units: set, prefer_ranged: bool = False, offense: bool = False,
                      siege_only: bool = False, domain: str = "Land", melee_only: bool = False) -> Optional[str]:
        """The best military unit this city can build, for a role."""
        P = self.p
        ranged_w = P["unit_ranged_pref"] if prefer_ranged else P["unit_ranged_nopref"]
        best, best_v = None, -1.0
        era = None
        for name in units:
            ud = g.rules.units[name]
            if P["space_reserve"] and ud.get("requiredResource"):
                if era is None:
                    from ..engine import research
                    era = research.player_era(g, city.owner)
                if self._breaks_space_reserve(g, city.owner, name, era):
                    continue
            if not ud["_military"] or _is_recon(ud) or ud["_domain"] != domain:
                continue
            if ud["_umap"].get(U.NuclearWeapon) or ud["_umap"].get(U.SelfDestructs):
                continue
            ut = ud.get("unitType")
            if siege_only and ut != "Siege":
                continue
            if melee_only and ud["_ranged"]:
                continue
            v = max(ud.get("strength", 0), ud.get("rangedStrength", 0) * ranged_w)
            if ut == "Siege":
                v *= 1.0 if offense else P["unit_siege_defensive"]
            if ut in ("Mounted", "Armored") and not offense:
                v *= P["unit_mobile_defensive"]
            v /= (ud.get("cost", 40) ** P["unit_cost_exp"])
            if v > best_v:
                best, best_v = name, v
        return best

    def city_bombard(self, g: Game, pid: int):
        """Fire every city that has a target in range.

        Free damage that costs nothing but the call, and the most commonly forgotten action in the game -
        by models as much as by people.
        """
        from ..engine.briefing import bombard_targets
        for c, idx, dmg, kills in bombard_targets(g, pid):
            x, y = g.grid.xy(idx)
            self.ex(g, pid, "city_attack", city_id=c.id, x=x, y=y)

    # ------------------------------------------------------------------
    # gold
    # ------------------------------------------------------------------
    def manage_gold(self, g: Game, pid: int, ctx: dict):
        """Spend surplus gold: buying units and buildings, and courting city-states.

        Hoarding gold is one of the clearest failures of a naive bot, and one of the easiest to fix.
        """
        from ..engine import cities as cm, units as unitmod, city_states as CS
        P = self.p
        p = g.player(pid)
        cities = ctx["cities"]
        for c in sorted(cities, key=lambda c: -ctx["threat"][c.id]):
            if not self.in_danger(g, c, ctx):
                break
            if g.military_at(c.idx) is None:
                unit = self.best_military(g, c, set(cm.buildable_items(g, c)["units"]), prefer_ranged=True)
                if unit:
                    self.ex(g, pid, "buy", city_id=c.id, item=unit)
        if ctx["gpt"] < 0 and p.gold + ctx["gpt"] * P["deficit_horizon"] < 0:
            for u in self._spare_units(g, pid, ctx)[: max(1, int(-ctx["gpt"]))]:
                self.ex(g, pid, "unit_order", unit_id=u.id, order="disband")
            if cities and ctx["gpt"] < -P["gold_focus_deficit"]:
                big = max(cities, key=lambda c: c.pop)
                if big.focus != "gold":
                    self.ex(g, pid, "set_city_focus", city_id=big.id, focus="gold")
        elif ctx["gpt"] > P["gold_focus_release"]:
            for c in cities:
                if c.focus == "gold":
                    self.ex(g, pid, "set_city_focus", city_id=c.id, focus="balanced")
        reserve = P["gold_reserve"] + P["gold_reserve_per_era"] * ctx["era"]
        for u in ctx["military"]:
            target, err, cost = unitmod.check_upgrade(g, u)
            if target and err is None and p.gold - cost > reserve \
                    and not self._breaks_space_reserve(g, pid, target, ctx["era"]):
                self.ex(g, pid, "upgrade_unit", unit_id=u.id)
        for c in sorted(cities, key=lambda c: cm.city_stats(g, c)["total"]["production"]):
            head = cm.current_construction(c)
            if not head or head in cm.PERPETUAL or c.puppet:
                continue
            reason, cost = cm.purchase_check(g, c, head, "Gold")
            if reason is not None or not cost:
                continue
            settler = head in g.rules.units and g.rules.units[head]["_umap"].get(U.FoundCity)
            worker = head in g.rules.units and g.rules.units[head]["_umap"].get(U.BuildImprovements)
            spare = p.gold - reserve
            cap = max(P["buy_cap"] + P["buy_cap_per_era"] * ctx["era"] + (P["buy_cap_settler"] if settler else 0),
                      spare * P["buy_cap_spare_share"])
            if cost <= spare and cost <= cap and (settler or worker or head in g.rules.buildings or ctx["wars"]) \
                    and not (head in g.rules.buildings and g.rules.buildings[head].get("isWonder")):
                if self.ex(g, pid, "buy", city_id=c.id, item=head) is not None:
                    self.manage_cities(g, pid)         # refill the emptied queue
        # court a city-state with spare gold
        if P["cs_gift_mode"] == "typed":
            self._gift_city_state(g, pid, ctx)
        elif p.gold > P["cs_gift_gold"] + P["cs_gift_gold_per_era"] * ctx["era"] and not ctx["wars"]:
            css = [q for q in g.city_states() if g.has_met(pid, q.id) and not g.at_war(pid, q.id)]
            if css:
                q = max(css, key=lambda q: (CS.influence(g, q.id, pid) >= P["cs_gift_focus_influence"],
                                            CS.influence(g, q.id, pid)))
                self.ex(g, pid, "city_state_action", player_id=q.id, action="gift_gold", amount=P["cs_gift_amount"])
        if g.religion_enabled:
            self._spend_faith(g, pid, ctx)

    def _gift_city_state(self, g: Game, pid: int, ctx: dict):
        """Gift gold where it buys the most: the city-state type we need (Mercantile when unhappy, Maritime for
        growth, Cultured for policies) and where the gift crosses the friend or ally threshold or keeps an alliance."""
        from ..engine import city_states as CS
        P = self.p
        p = g.player(pid)
        reserve = P["cs_typed_reserve"] + P["cs_typed_reserve_per_era"] * ctx["era"]
        if p.gold < reserve + P["cs_typed_min_spare"] or ctx["wars"]:
            return
        need_hap = ctx["hap"] < len(ctx["cities"]) + P["cs_typed_hap_margin"]
        type_w = {"Mercantile": P["cs_w_mercantile_unhappy"] if need_hap else P["cs_w_mercantile"],
                  "Maritime": P["cs_w_maritime"], "Cultured": P["cs_w_cultured"], "Religious": P["cs_w_religious"],
                  "Militaristic": P["cs_w_militaristic"]}
        amount = P["cs_typed_amount"] if p.gold - reserve < P["cs_typed_big_above"] else P["cs_typed_big_amount"]
        best, best_v = None, 0.0
        for q in g.city_states():
            if not q.alive or not g.has_met(pid, q.id) or g.at_war(pid, q.id):
                continue
            inf = CS.influence(g, q.id, pid)
            gain = CS.influence_from_gold(g, q.id, pid, amount)
            after = inf + gain
            rivals = max((CS.influence(g, q.id, o.id) for o in g.majors() if o.id != pid and o.alive), default=0)
            v = 0.0
            if inf < CS.FRIEND <= after:
                v += P["cs_v_friend"]
            if after >= CS.ALLY and (q.ally != pid) and after > rivals:
                v += P["cs_v_ally"]
            if q.ally == pid and rivals > inf - P["cs_typed_defend_margin"]:
                v += P["cs_v_defend"]                      # defend the alliance
            if q.ally == pid and inf - rivals > P["cs_typed_safe_lead"] and inf > CS.ALLY + P["cs_typed_safe_above_ally"]:
                v = 0.0                                    # safely allied: no need to pay more
            v *= type_w.get(q.cs_type, 1.0)
            if v > best_v:
                best, best_v = q, v
        if best is not None:
            self.ex(g, pid, "city_state_action", player_id=best.id, action="gift_gold", amount=amount)

    def choose_beliefs(self, g: Game, pid: int, needed: dict) -> list[str]:
        """Pick religious beliefs, weighted by what this civilization can use."""
        from ..engine import religion
        P = self.p
        if P["belief_mode"] != "prefs":
            return religion.ai_choose_beliefs(g, pid, needed)
        prefs = {"Pantheon": P["beliefs_pantheon"], "Founder": P["beliefs_founder"],
                 "Follower": P["beliefs_follower"], "Enhancer": P["beliefs_enhancer"]}
        out, taken = [], religion.beliefs_taken(g)
        for t, n in needed.items():
            pool = [b for b in religion.beliefs_available(g, t) if b not in out and b not in taken]
            rank = prefs.get(t) or [x for v in prefs.values() for x in v]
            pool.sort(key=lambda b: (rank.index(b) if b in rank else 99, b))
            out.extend(pool[:n])
        return out

    def _spend_faith(self, g: Game, pid: int, ctx: dict):
        """Spend faith on units and buildings once there is enough."""
        from ..engine import cities as cm
        P = self.p
        p = g.player(pid)
        if p.religion_state not in ("religion", "enhancing", "enhanced") or p.faith < P["faith_min"]:
            return
        if P["faith_buildings"]:
            # buildings our beliefs let us buy with faith (Pagoda, Mosque, Cathedral...) before more missionaries
            for c in sorted(ctx["cities"], key=lambda c: -c.pop):
                for name, bd in g.rules.buildings.items():
                    if bd.get("cost", 1) != 0 or name in c.buildings or bd.get("isWonder"):
                        continue
                    reason, cost = cm.purchase_check(g, c, name, "Faith")
                    if reason is None and cost and p.faith - cost > P["faith_building_keep"]:
                        if self.ex(g, pid, "buy", city_id=c.id, item=name, currency="Faith") is not None:
                            return
        for c in ctx["cities"]:
            for name, ud in g.rules.units.items():
                if ud["_umap"].get(U.CanSpreadReligion) and not ud["_umap"].get(U.MayFoundReligion):
                    reason, cost = cm.purchase_check(g, c, name, "Faith")
                    if reason is None and cost and p.faith - cost > P["faith_missionary_keep"]:
                        missionaries = sum(1 for u in ctx["units"] if u.type == name)
                        if missionaries < P["max_missionaries"]:
                            self.ex(g, pid, "buy", city_id=c.id, item=name, currency="Faith")
                        return

    def _spare_units(self, g: Game, pid: int, ctx: dict) -> list:
        """Units not committed to defence or a war, available for something else."""
        P = self.p
        out = []
        for u in ctx["units"]:
            ud = _ud(g, u)
            if not ud["_military"]:
                continue
            city = g.city_at(u.idx)
            if city and city.owner == pid and ctx["threat"].get(city.id, 0) > 0:
                continue
            value = 0 if _is_recon(ud) else _power(ud)
            if ud.get("obsoleteTech") and g.has_tech(pid, ud["obsoleteTech"]):
                value *= P["spare_obsolete_mult"]
            if city and city.owner == pid and g.military_at(u.idx) is u:
                value += P["spare_garrison_value"]
            out.append((value, u))
        out.sort(key=lambda t: t[0])
        keep = max(len(ctx["cities"]) * P["keep_units_per_city"], 1)
        left = len([1 for v, u in out if not _is_recon(_ud(g, u))])
        result = []
        for v, u in out:
            if not _is_recon(_ud(g, u)):
                if left <= keep:
                    break
                left -= 1
            result.append(u)
        return result

    # ------------------------------------------------------------------
    # units
    # ------------------------------------------------------------------
    def manage_units(self, g: Game, pid: int, ctx: Optional[dict] = None):
        """Give every unit its orders for the turn."""
        ctx = ctx or self.context(g, pid)
        self._garrisons = {c.id: m.id for c in ctx["cities"] for m in [g.military_at(c.idx)]
                           if m and m.owner == pid and _ud(g, m)["_domain"] == "Land" and self.needs_garrison(c, ctx)}
        order = sorted(g.player_units(pid), key=lambda u: (0 if _ud(g, u)["_ranged"] else 1, u.id))
        for u in order:
            if g.unit(u.id) is None or g.s.current != pid:
                continue
            ud = _ud(g, u)
            try:
                if ud["_umap"].get(U.FoundCity):
                    self.handle_settler(g, pid, u, ctx)
                elif ud["_umap"].get(U.BuildImprovements):
                    if u.activity not in ("automate", "build"):
                        self.ex(g, pid, "unit_order", unit_id=u.id, order="automate")
                elif ud["_umap"].get(U.CreateWaterImprovements):
                    self.handle_work_boat(g, pid, u)
                elif ud["_great_person"] or ud["_umap"].get(U.CanSpreadReligion) \
                        or ud["_umap"].get(U.AddInCapital) or ud["_umap"].get(U.CanRemoveHeresy):
                    self.handle_special(g, pid, u, ctx)
                elif _is_recon(ud):
                    self.handle_scout(g, pid, u, ctx)
                elif ud["_domain"] == "Air":
                    self.handle_air(g, pid, u)
                elif ud["_domain"] == "Water":
                    self.handle_naval(g, pid, u, ctx)
                elif ud["_military"]:
                    self.handle_military(g, pid, u, ctx)
            except ActionError:
                pass
            u = g.unit(u.id)
            if u is not None:
                self._promote(g, pid, u)

    def _promote(self, g: Game, pid: int, u):
        """Take a promotion when a unit has earned one."""
        from ..engine import units as unitmod
        in_city, lines = tuple(self.p["promo_in_city"]), tuple(self.p["promo_lines"])
        guard = 0
        while unitmod.can_promote(g, u) and guard < 5:
            guard += 1
            opts = sorted(unitmod.available_promotions(g, u))
            if not opts:
                break
            city = g.city_at(u.idx)
            pick = None
            if city and city.owner == pid:
                pick = next((o for o in opts if o.startswith(in_city)), None)
            pick = pick or next((o for o in opts if o.startswith(lines)), None) or opts[0]
            if self.ex(g, pid, "promote_unit", unit_id=u.id, promotion=pick) is None:
                break

    def _is_garrison(self, u) -> bool:
        """Whether a unit is assigned to garrison a city."""
        return u.id in self._garrisons.values()

    def _move(self, g: Game, pid: int, u, idx: int):
        """Move a unit toward a tile. (Returns None, as a refused move would, when it is already there.)"""
        if idx == u.idx:
            return None
        x, y = g.grid.xy(idx)
        return self.ex(g, pid, "move_unit", unit_id=u.id, x=x, y=y)

    def handle_settler(self, g: Game, pid: int, u, ctx: dict):
        """Send a settler to a site and found a city when it arrives."""
        from ..engine import cities as cm
        P = self.p
        if not ctx["cities"] and cm.found_check(g, pid, u.idx) is None:
            self.ex(g, pid, "unit_action", unit_id=u.id, action="found_city")
            return
        if u.moves <= 0:
            return                  # already moved this turn (standing goto orders run at the start of the turn)
        sites = self.expansion_sites(g, pid, ctx)
        target = u.goto if u.goto is not None and cm.found_check(g, pid, u.goto) is None else None
        if target is None and sites:
            target = min(sites, key=lambda s: g.grid.distance(s, u.idx) + sites.index(s) * P["settler_rank_cost"])
        if target is None:
            if cm.found_check(g, pid, u.idx) is None and ctx["cities"] and \
                    min(g.grid.distance(c.idx, u.idx) for c in ctx["cities"]) <= P["settler_found_here_dist"]:
                self.ex(g, pid, "unit_action", unit_id=u.id, action="found_city")
            elif not g.city_at(u.idx) and ctx["cities"]:
                self._move(g, pid, u, min(ctx["cities"], key=lambda c: g.grid.distance(c.idx, u.idx)).idx)
            return
        if u.idx == target:
            self.ex(g, pid, "unit_action", unit_id=u.id, action="found_city")
            return
        # danger = a hostile unit that could reach the settler (or the site) next turn; an escort sharing the tile
        # makes the trip safe (UnCiv escorts settlers the same way)
        from ..engine.automation import _threat_reach
        danger = any(min(g.grid.distance(e.idx, u.idx), g.grid.distance(e.idx, target))
                     <= _threat_reach(g, e) + P["settler_danger_margin"] for e in ctx["hostile"])
        if danger and not self._guarded(g, pid, u):
            esc = self._assign_escort(g, pid, u, ctx)
            if esc is None or not self._guarded(g, pid, u):
                key = (pid, target)
                self._retreats[key] = self._retreats.get(key, 0) + 1
                if self._retreats[key] >= P["settler_max_retreats"]:   # this site keeps being unsafe: pick another
                    self._bad_sites[key] = g.turn
                    self._sites_cache.pop(pid, None)
                    self._retreats[key] = 0
                    u.goto = None
                if g.city_at(u.idx) is None and ctx["cities"]:
                    self._move(g, pid, u, min(ctx["cities"], key=lambda c: g.grid.distance(c.idx, u.idx)).idx)
                else:
                    self._need_escort[pid] = u.idx
                    self.ex(g, pid, "unit_order", unit_id=u.id, order="skip")
                return
        r = self._move(g, pid, u, target)
        self._follow(g, pid, u)
        if r is None:
            from ..engine import movement
            if g.unit(u.id) is not None and movement.find_path(g, u, target) is None:
                self._bad_sites[(pid, target)] = g.turn       # unreachable: try elsewhere for a while
                self._sites_cache.pop(pid, None)
                u.goto = None
        elif r.get("arrived") and g.unit(u.id) and u.moves > 0:
            self.ex(g, pid, "unit_action", unit_id=u.id, action="found_city")

    def _knows_rival_city(self, g: Game, pid: int) -> bool:
        """Whether this civilization has seen a rival city, which changes how it expands."""
        p = g.player(pid)
        return any(c.owner != pid and g.player(c.owner).kind == "major" and p.explored[c.idx]
                   for c in g.s.cities.values())

    def _guarded(self, g: Game, pid: int, u) -> bool:
        """Whether a civilian has an escort on its tile."""
        m = g.military_at(u.idx)
        return m is not None and m.owner == pid

    def _assign_escort(self, g: Game, pid: int, u, ctx: dict):
        """Bring a free (non-garrison) land military unit onto the settler's tile."""
        P = self.p
        esc = g.unit(self._escorts.get(u.id)) if self._escorts.get(u.id) is not None else None
        if esc is None:
            free = [m for m in ctx["military"] if _ud(g, m)["_domain"] == "Land" and not self._is_garrison(m)
                    and m.id not in self._escorts.values() and m.hp >= P["escort_min_hp"]
                    and g.grid.distance(m.idx, u.idx) <= P["escort_radius"]]
            if not free:
                return None
            esc = min(free, key=lambda m: g.grid.distance(m.idx, u.idx))
            self._escorts[u.id] = esc.id
        if esc.idx != u.idx and esc.moves > 0:
            self._move(g, pid, esc, u.idx)
        return esc

    def _follow(self, g: Game, pid: int, u):
        """The settler moved: its escort (if any) moves onto the same tile."""
        eid = self._escorts.get(u.id)
        esc = g.unit(eid) if eid is not None else None
        if esc is None or g.unit(u.id) is None:
            self._escorts.pop(u.id, None)
            return
        if esc.idx != u.idx and esc.moves > 0:
            self._move(g, pid, esc, u.idx)

    def handle_work_boat(self, g: Game, pid: int, u):
        """Send a work boat to the nearest sea resource worth improving."""
        from ..engine.actions import unit_actions
        from ..engine import tiles as T
        acts = [a for a in unit_actions(g, u) if a["id"].startswith("create:") and a["available"]]
        if acts:
            self.ex(g, pid, "unit_action", unit_id=u.id, action=acts[0]["id"])
            return
        best = None
        for idx in g.grid.within(u.idx, self.p["boat_seek_radius"]):
            t = g.s.tiles[idx]
            if t.owner == pid and t.resource and g.is_water(idx) and not t.improvement \
                    and T.resource_visible(g, pid, t.resource):
                best = idx
                break
        if best is None:
            self.ex(g, pid, "unit_order", unit_id=u.id, order="disband")
            return
        self._move(g, pid, u, best)
        if g.unit(u.id) and u.idx == best:
            acts = [a for a in unit_actions(g, u) if a["id"].startswith("create:") and a["available"]]
            if acts:
                self.ex(g, pid, "unit_action", unit_id=u.id, action=acts[0]["id"])

    def handle_special(self, g: Game, pid: int, u, ctx: dict):
        """Great people, religious units and spaceship parts: use the best available action."""
        from ..engine.actions import unit_actions
        from ..engine import religion
        ud = _ud(g, u)
        p = g.player(pid)
        acts = {a["id"]: a for a in unit_actions(g, u)}
        ok = {k for k, a in acts.items() if a["available"]}
        cap = g.city(p.capital) if p.capital is not None else None
        if "add_to_spaceship" in acts:
            if "add_to_spaceship" in ok:
                self.ex(g, pid, "unit_action", unit_id=u.id, action="add_to_spaceship")
            elif cap is not None:
                self._clear_civilian_slot(g, pid, cap.idx, keep=u)
                self._move(g, pid, u, cap.idx)
            return
        if "found_religion" in acts:
            if "found_religion" in ok:
                need = religion.beliefs_to_choose(g, pid, enhancing=False)
                beliefs = self.choose_beliefs(g, pid, need)
                self.ex(g, pid, "unit_action", unit_id=u.id, action="found_religion",
                        name=f"Faith of {p.name}", beliefs=beliefs)
                return
            if religion.can_found_religion(g, pid) is None:
                # any own city will do (UnCiv); pick the nearest one without another civilian in it
                free = [c for c in ctx["cities"] if not any(x.id != u.id and not _ud(g, x)["_military"]
                                                              for x in g.units_at(c.idx))]
                if free:
                    self._move(g, pid, u, min(free, key=lambda c: g.grid.distance(c.idx, u.idx)).idx)
                    return
        if "enhance_religion" in ok:
            need = religion.beliefs_to_choose(g, pid, enhancing=True)
            self.ex(g, pid, "unit_action", unit_id=u.id, action="enhance_religion",
                    beliefs=self.choose_beliefs(g, pid, need))
            return
        if "enhance_religion" in acts and p.religion_state == "religion" and g.city_at(u.idx) is None and ctx["cities"]:
            self._move(g, pid, u, min(ctx["cities"], key=lambda c: g.grid.distance(c.idx, u.idx)).idx)
            return
        for a in ("hurry_research", "trade_mission", "hurry_construction"):
            if a in ok and (a != "hurry_construction" or g.city_at(u.idx).queue):
                self.ex(g, pid, "unit_action", unit_id=u.id, action=a)
                return
        trig = [k for k in ok if k.startswith("trigger:")]
        if trig:
            self.ex(g, pid, "unit_action", unit_id=u.id, action=trig[0])
            return
        if ("spread_religion" in acts or "remove_heresy" in acts) and not u.religion and "remove_heresy" not in acts:
            if u.activity is None:
                self.ex(g, pid, "unit_order", unit_id=u.id, order="sleep")
            return
        if "spread_religion" in acts or "remove_heresy" in acts:
            here = g.city(g.s.tiles[u.idx].city) if g.s.tiles[u.idx].city is not None else None
            if "spread_religion" in ok and here is not None and religion.majority_religion(g, here) != u.religion:
                self.ex(g, pid, "unit_action", unit_id=u.id, action="spread_religion")
                return
            target = None
            for c in sorted(g.s.cities.values(), key=lambda c: g.grid.distance(c.idx, u.idx)):
                if (c.owner == pid or g.player(c.owner).kind != "barbarian") and not g.at_war(pid, c.owner) \
                        and religion.majority_religion(g, c) != u.religion and p.explored[c.idx]:
                    target = c
                    break
            if target is not None and g.grid.distance(target.idx, u.idx) > 1:
                near = [n for n in g.grid.neighbors(target.idx) if not g.units_at(n) and not g.is_water(n)]
                if near:
                    self._move(g, pid, u, min(near, key=lambda n: g.grid.distance(n, u.idx)))
            return
        if ud.get("unitType") == "Civilian":
            creates = [k for k in ok if k.startswith("create:")]
            if creates and g.s.tiles[u.idx].owner == pid and not g.city_at(u.idx):
                self.ex(g, pid, "unit_action", unit_id=u.id, action=creates[0])
                return
            # walk to a free tile next to the capital to build the great improvement there
            if cap is not None:
                spots = [i for i in g.grid.within(cap.idx, 2)[1:] if g.s.tiles[i].owner == pid and not g.is_water(i)
                         and not g.s.tiles[i].improvement and not g.units_at(i)]
                if spots:
                    self._move(g, pid, u, min(spots, key=lambda i: g.grid.distance(i, u.idx)))
                    return
        # great generals and admirals follow the army
        army = [m for m in ctx["military"] if _ud(g, m)["_domain"] == _ud(g, u)["_domain"]]
        if army:
            r = self.p["general_front_radius"]
            front = min(army, key=lambda m: -len([e for e in ctx["hostile"] if g.grid.distance(e.idx, m.idx) <= r]))
            if g.grid.distance(front.idx, u.idx) > 1:
                self._move(g, pid, u, front.idx if front.idx != u.idx else u.idx)
        elif u.activity is None:
            self.ex(g, pid, "unit_order", unit_id=u.id, order="sleep")

    def _clear_civilian_slot(self, g: Game, pid: int, idx: int, keep):
        """A tile holds one civilian: move our other civilian (a parked general, a missionary) off `idx` so `keep`
        can enter. Spaceship parts must be in the capital to be added, and one sleeping unit there used to block
        every part for the rest of the game."""
        if g.grid.distance(keep.idx, idx) > 2:
            return
        from ..engine import movement
        for x in list(g.units_at(idx)):
            xd = _ud(g, x)
            if x.owner != pid or x.id == keep.id or xd["_military"] or xd["_umap"].get(U.SpaceshipPart) or x.moves <= 0:
                continue
            spots = [n for n in g.grid.neighbors(idx) if not g.is_water(n) and movement.can_stand(g, pid, xd, n, x)]
            if spots:
                self._move(g, pid, x, spots[0])

    def handle_scout(self, g: Game, pid: int, u, ctx: dict):
        """Keep a scout exploring."""
        if u.activity == "explore":
            return
        r = self.ex(g, pid, "unit_order", unit_id=u.id, order="explore")
        if r is None or not r.get("exploring"):
            if len(ctx["units"]) >= ctx["supply"] or g.turn > self.p["scout_disband_turn"]:
                self.ex(g, pid, "unit_order", unit_id=u.id, order="disband")
            else:
                self.ex(g, pid, "unit_order", unit_id=u.id, order="sleep")

    def handle_air(self, g: Game, pid: int, u):
        """Base and use aircraft."""
        from ..engine import units as unitmod
        for idx in g.grid.within(u.idx, unitmod.attack_range(g, u)):
            if idx == u.idx:
                continue
            owner = None
            c = g.city_at(idx)
            m = g.military_at(idx)
            if c is not None:
                owner = c.owner
            elif m is not None:
                owner = m.owner
            if owner is not None and g.at_war(pid, owner):
                x, y = g.grid.xy(idx)
                if self.ex(g, pid, "attack", unit_id=u.id, x=x, y=y):
                    return
        if u.activity != "sleep":
            self.ex(g, pid, "unit_order", unit_id=u.id, order="sleep")

    def handle_naval(self, g: Game, pid: int, u, ctx: dict):
        """Fight with, or reposition, a ship."""
        if self._attack_best(g, pid, u):
            return
        if u.hp < self.p["naval_heal_hp"]:
            if u.activity != "heal":
                self.ex(g, pid, "unit_order", unit_id=u.id, order="heal")
            return
        if u.activity is None:
            self.ex(g, pid, "unit_order", unit_id=u.id, order="sleep")

    def _attack_best(self, g: Game, pid: int, u, allow_city_capture: bool = True) -> bool:
        """Attack the best target in reach, if attacking is worthwhile.

        Predicted damage decides: a unit that would lose the exchange holds instead, which is the
        difference between an army and a queue of casualties.
        """
        from ..engine import combat, units as unitmod
        P = self.p
        ud = _ud(g, u)
        plan = self._war_plan.get(pid)
        siege_city = plan["city"] if plan and plan.get("advance") else None
        if u.moves <= 0 or combat.can_attack_now(g, u) is not None:
            return False
        radius = unitmod.attack_range(g, u) if ud["_ranged"] else 1
        best, best_v = None, 0.0
        for idx in g.grid.within(u.idx, radius)[1:]:
            try:
                pv = combat.preview(g, u, idx)
            except ActionError:
                continue
            dd = sum(pv["damage_to_defender"]) / 2
            da = sum(pv["damage_to_attacker"]) / 2
            if pv["target"] == "city":
                if pv.get("note"):
                    if not ud["_ranged"] and allow_city_capture:
                        best, best_v = idx, 10000
                        break
                    continue
                v = dd * (P["atk_siege_city_mult"] if ud.get("unitType") == "Siege" else 1.0) - da * P["atk_city_loss_weight"]
                if idx == siege_city:
                    v *= P["siege_city_pref"]          # focus fire on the city we are besieging
                if not ud["_ranged"] and u.hp - da < P["atk_city_min_hp"]:
                    continue
            else:
                kill = dd >= pv["defender_hp"]
                v = dd * (P["atk_kill_mult"] if kill else 1) - da * P["atk_unit_loss_weight"]
                if not ud["_ranged"] and u.hp - da < P["atk_unit_min_hp"]:
                    continue
            if v > best_v:
                best, best_v = idx, v
        if best is None:
            return False
        x, y = g.grid.xy(best)
        return self.ex(g, pid, "attack", unit_id=u.id, x=x, y=y) is not None

    def handle_military(self, g: Game, pid: int, u, ctx: dict):
        """Give a land unit its orders: defend, besiege, escort or advance."""
        from ..engine import movement
        P = self.p
        ud = _ud(g, u)
        if u.moves <= 0:
            return
        garrison = self._is_garrison(u)
        ward = next((sid for sid, eid in self._escorts.items() if eid == u.id), None)
        if ward is not None:
            if g.unit(ward) is not None:
                if g.unit(u.id) is not None and g.unit(ward) is not None and u.idx != g.unit(ward).idx and u.moves > 0:
                    self._move(g, pid, u, g.unit(ward).idx)
                return
            self._escorts.pop(ward, None)
        # ranged units in an active siege first move to a firing position on the target city
        plan = self._war_plan.get(pid) if ctx["wars"] else None
        if plan and plan.get("advance") and ud["_ranged"] and not garrison and P["siege_move_first"]:
            from ..engine import units as unitmod
            d = g.grid.distance(u.idx, plan["city"])
            rng = unitmod.attack_range(g, u)
            if 1 < d <= rng + P["siege_approach_extra"]:
                from ..engine import visibility
                if d > rng or not visibility.has_los(g, u.idx, plan["city"]):
                    dest = self._approach_tile(g, pid, u, plan["city"], siege=True)
                    if dest is not None and dest != u.idx:
                        self._move(g, pid, u, dest)
                        if g.unit(u.id) is None or u.moves <= 0:
                            return
        if self._attack_best(g, pid, u):
            if g.unit(u.id) is None or u.moves <= 0 or garrison:
                return
        if garrison:
            if u.activity is None:
                self.ex(g, pid, "unit_order", unit_id=u.id, order="fortify")
            return
        if u.hp < P["heal_below_hp"]:
            cities = [c for c in ctx["cities"] if g.military_at(c.idx) is None]
            if cities and not g.city_at(u.idx):
                c = min(cities, key=lambda c: g.grid.distance(c.idx, u.idx))
                if g.grid.distance(c.idx, u.idx) <= P["heal_city_radius"]:
                    self._move(g, pid, u, c.idx)
                    if g.unit(u.id) and u.idx == c.idx:
                        self._garrisons[c.id] = u.id
                    return
            if u.activity not in ("heal", "fortify"):
                self.ex(g, pid, "unit_order", unit_id=u.id, order="heal")
            return
        empty = [c for c in ctx["cities"] if c.id not in self._garrisons and self.needs_garrison(c, ctx)]
        if empty:
            c = min(empty, key=lambda c: g.grid.distance(c.idx, u.idx))
            if g.grid.distance(c.idx, u.idx) <= P["garrison_radius"]:
                self._garrisons[c.id] = u.id
                if u.idx != c.idx:
                    self._move(g, pid, u, c.idx)
                if g.unit(u.id) and u.idx == c.idx:
                    self.ex(g, pid, "unit_order", unit_id=u.id, order="fortify")
                return
        campaigning = bool(ctx["wars"]) and pid in self._war_plan
        threatened = [c for c in ctx["cities"]
                      if ctx["threat"][c.id] > (self.city_defense(g, c) * P["campaign_defend_ratio"] if campaigning else 0)]
        if threatened:
            c = max(threatened, key=lambda c: ctx["threat"][c.id] - g.grid.distance(c.idx, u.idx) * P["defend_distance_cost"])
            if g.grid.distance(c.idx, u.idx) <= (P["defend_radius_campaign"] if campaigning else P["defend_radius"]):
                enemies = ctx["near_enemies"][c.id]
                target = min(enemies, key=lambda e: g.grid.distance(e.idx, u.idx)).idx if enemies else c.idx
                dest = self._approach_tile(g, pid, u, target)
                if dest is not None:
                    self._move(g, pid, u, dest)
                    self._attack_best(g, pid, u)
                return
        if ctx["wars"] and ud["_domain"] == "Land":
            plan = self._war_target(g, pid, ctx)
            if plan is not None:
                if not plan["advance"]:
                    dest = plan["rally"] if g.grid.distance(u.idx, plan["rally"]) > P["rally_radius"] else u.idx
                    if dest != u.idx and not movement.can_stand(g, pid, ud, dest, u):
                        dest = self._approach_tile(g, pid, u, plan["rally"], ring=1)
                else:
                    ranged = ud["_ranged"]
                    close_in = ranged or plan["siege_ready"]
                    dest = self._approach_tile(g, pid, u, plan["city"], siege=ranged,
                                               ring=0 if close_in else P["melee_standoff"])
                if dest is not None and dest != u.idx:
                    self._move(g, pid, u, dest)
                    self._attack_best(g, pid, u)
                    return
                if dest == u.idx:
                    if u.activity is None and not plan["advance"]:
                        self.ex(g, pid, "unit_order", unit_id=u.id, order="fortify")
                    return
        prep = self._war_prep.get(pid)
        if not ctx["wars"] and prep and prep.get("rally") is not None and ud["_domain"] == "Land":
            rally = prep["rally"]
            if g.grid.distance(u.idx, rally) > P["rally_radius"]:
                dest = rally if movement.can_stand(g, pid, ud, rally, u) else self._approach_tile(g, pid, u, rally, ring=1)
                if dest is not None and dest != u.idx:
                    self._move(g, pid, u, dest)
                    return
            elif u.activity is None:
                self.ex(g, pid, "unit_order", unit_id=u.id, order="fortify")
            return
        camp = self._camp_target(g, pid, u, ctx)
        if camp is not None:
            defender = g.military_at(camp)
            dest = camp if defender is None else self._approach_tile(g, pid, u, camp)
            if dest is not None and dest != u.idx and self._move(g, pid, u, dest):
                self._attack_best(g, pid, u)
                return
        ruins = self._ruin_target(g, pid, u)
        if ruins is not None and self._move(g, pid, u, ruins):
            return
        g.player(pid)
        if g.turn > P["seek_after_turn"] and not self._knows_rival_city(g, pid) \
                and not any(x.activity == "explore" and _ud(g, x)["_military"] and not _is_recon(_ud(g, x))
                            for x in ctx["units"]):
            r = self.ex(g, pid, "unit_order", unit_id=u.id, order="explore")
            if r and r.get("exploring"):
                return
        if not g.city_at(u.idx) and ctx["cities"]:
            c = min(ctx["cities"], key=lambda c: g.grid.distance(c.idx, u.idx))
            spot = c.idx if g.military_at(c.idx) is None else self._approach_tile(g, pid, u, c.idx)
            if spot is not None and spot != u.idx and g.grid.distance(spot, u.idx) > 1 and self._move(g, pid, u, spot):
                return
        if u.activity is None:
            self.ex(g, pid, "unit_order", unit_id=u.id, order="fortify")

    def _camp_target(self, g: Game, pid: int, u, ctx: dict) -> Optional[int]:
        """A barbarian camp worth clearing, if there is one."""
        P = self.p
        p = g.player(pid)
        best, best_d = None, 99
        for camp in g.s.camps.values():
            idx = camp["idx"]
            if camp.get("destroyed") or not p.explored[idx]:
                continue
            d_city = min((g.grid.distance(c.idx, idx) for c in ctx["cities"]), default=99)
            d = g.grid.distance(u.idx, idx)
            if d_city <= P["camp_city_radius"] and d <= P["camp_unit_radius"] and d < best_d:
                best, best_d = idx, d
        return best

    def _ruin_target(self, g: Game, pid: int, u) -> Optional[int]:
        """Ancient ruins worth walking to."""
        p = g.player(pid)
        for idx in g.grid.within(u.idx, self.p["ruin_radius"])[1:]:
            if p.explored[idx] and g.s.tiles[idx].improvement == "Ancient ruins" and not g.units_at(idx):
                return idx
        return None

    def _war_target(self, g: Game, pid: int, ctx: dict) -> Optional[dict]:
        """The city this civilization's war is aimed at, if it is at war by choice."""
        P = self.p
        plan = self._war_plan.get(pid)
        p = g.player(pid)
        if plan:
            c = g.city_at(plan["city"])
            if c is None or c.owner == pid or not g.at_war(pid, c.owner) or not ctx["cities"]:
                plan = None
        if plan is None:
            best, best_score = None, 999
            for c in g.s.cities.values():
                if c.owner in ctx["wars"] and p.explored[c.idx]:
                    d = min((g.grid.distance(c.idx, mc.idx) for mc in ctx["cities"]), default=99)
                    if d > P["war_target_max_dist"]:
                        continue                  # out of reach: this war is fought at home
                    score = d + c.pop * P["target_pop_weight"] - (P["target_damaged_bonus"] if c.damaged_turn >= g.turn - 1 else 0)
                    if score < best_score:
                        best, best_score = c, score
            if best is None:
                return None
            plan = {"city": best.idx, "since": g.turn, "advance": False, "checked": -1}
            self._war_plan[pid] = plan
        if plan.get("checked") != g.turn:
            from ..engine import cities as cm
            plan["checked"] = g.turn
            target = plan["city"]
            stage = min(ctx["cities"], key=lambda c: g.grid.distance(c.idx, target))
            if plan.get("rally") is None:
                plan["rally"] = self._rally_point(g, pid, target, stage.idx)
            field = [u for u in ctx["military"] if _ud(g, u)["_domain"] == "Land" and not self._is_garrison(u)]
            rally_d = g.grid.distance(plan["rally"], target)
            if plan["advance"]:
                gathered = [u for u in field if g.grid.distance(u.idx, target) <= P["gather_radius"]]
            else:
                gathered = [u for u in field if g.grid.distance(u.idx, plan["rally"]) <= P["gather_radius"]
                            or g.grid.distance(u.idx, target) <= rally_d]
            need = max(P["advance_min"], min(P["advance_max"], int(len(field) * P["advance_share"])))
            city = g.city_at(target)
            weak = city is not None and (city.health <= cm.max_health(g, city) * P["weak_city_health"]
                                         or self.military_power(g, city.owner) * P["weak_power_ratio"] < self.military_power(g, pid))
            if not plan["advance"] and (len(gathered) >= need or (weak and len(field) >= P["weak_min_units"])):
                plan["advance"] = True
            elif plan["advance"] and len(gathered) < P["advance_hold_min"] and not weak:
                plan["advance"] = False
                plan["rally"] = self._rally_point(g, pid, target, stage.idx)
            in_range = sum(1 for u in field if _ud(g, u)["_ranged"] and g.grid.distance(u.idx, target) <= P["siege_ready_dist"])
            plan["siege_ready"] = bool(city) and (in_range >= 1 or city.health <= cm.max_health(g, city) * P["siege_ready_health"])
        return plan

    def _rally_point(self, g: Game, pid: int, target: int, stage: int) -> int:
        """Where to gather an army before attacking, near the target but not yet in reach.

        Gathering first is what makes the bot's wars work at all. Units sent one at a time arrive one at a
        time and die one at a time.
        """
        P = self.p
        cont = g.s.continents
        best, best_v = stage, 1e9
        for idx in g.grid.within(target, P["rally_max_dist"]):
            d = g.grid.distance(idx, target)
            t = g.s.tiles[idx]
            if d < P["rally_min_dist"] or cont[idx] != cont[target] or g.city_at(idx) or t.owner not in (None, pid):
                continue
            v = g.grid.distance(idx, stage) - (P["rally_cover_bonus"] if t.hills or t.feature in ("Forest", "Jungle") else 0)
            if v < best_v:
                best, best_v = idx, v
        return best

    def _approach_tile(self, g: Game, pid: int, u, target: int, siege: bool = False, ring: int = 0) -> Optional[int]:
        """The tile a unit should move to next when closing on a target."""
        from ..engine import movement, visibility, units as unitmod
        ud = _ud(g, u)
        want = ring or (2 if siege and ud.get("range", 0) >= 2 else 1)
        # a ranged unit needs line of sight to shoot from 2+ tiles away (unless it has indirect fire)
        needs_los = siege and ud["_ranged"] and not unitmod.unit_has(g, u, U.IndirectFire, with_civ=True)

        def sees(n):
            """Whether a tile has line of sight to the target, for siege units that need it."""
            return not needs_los or g.grid.distance(n, target) <= 1 or visibility.has_los(g, n, target)
        if g.grid.distance(u.idx, target) <= want and sees(u.idx):
            return u.idx
        opts = []
        for r in ([want, 1] if want > 1 and not ring else [want]):
            opts = [n for n in g.grid.ring(target, r)
                    if (n == u.idx or movement.can_stand(g, pid, ud, n, u)) and sees(n)]
            if opts:
                break
        if not opts:
            opts = [n for n in g.grid.within(target, self.p["approach_fallback_radius"])[1:]
                    if movement.can_stand(g, pid, ud, n, u)]
        if not opts:
            return None
        return min(opts, key=lambda n: (g.grid.distance(n, u.idx), -g.s.tiles[n].hills))

    # ------------------------------------------------------------------
    # diplomacy
    # ------------------------------------------------------------------
    def military_power(self, g: Game, pid: int) -> float:
        """This civilization's military strength, for comparing against neighbours."""
        from ..engine.victory import military_strength
        return military_strength(g, pid) + 1

    def consider_diplomacy(self, g: Game, pid: int, ctx: Optional[dict] = None):
        """Decide on wars, peace, denunciations and friendships."""
        from ..engine import diplomacy as D
        P = self.p
        a = self.aggression
        ctx = ctx or self.context(g, pid)
        p = g.player(pid)
        mine = self.military_power(g, pid)
        for q in list(p.met):
            if not g.player(q).alive or g.player(q).kind != "major":
                continue
            rel = g.relation(pid, q)
            if rel is None:
                continue
            theirs = self.military_power(g, q)
            if rel["war"]:
                lost = sum(1 for c in g.s.cities.values() if c.founder == pid and c.owner == q)
                long_war = g.turn - rel["since"] >= P["war_long_turns"]
                losing = mine < theirs * P["losing_ratio"] or lost
                plan = self._war_plan.get(pid)
                tc = g.city_at(plan["city"]) if plan else None
                if tc is not None and tc.owner == q and not lost and mine >= theirs * P["losing_ratio"]:
                    from ..engine import cities as cm
                    near = sum(1 for m in ctx["military"] if g.grid.distance(m.idx, tc.idx) <= P["siege_progress_radius"])
                    if near >= P["siege_progress_units"] and tc.health < cm.max_health(g, tc) * P["siege_progress_health"]:
                        long_war = False            # the siege is progressing: keep going
                if g.turn - rel["since"] >= P["peace_min_turns"] and (losing or long_war) \
                        and self.rng.random() < P["peace_offer_chance"]:
                    self.ex(g, pid, "open_negotiation", to=q, message="This war profits no one. Let us make peace.",
                            give=[{"type": "peace_treaty"}], receive=[])
                continue
            every = P["diplo_every"]
            if g.turn % every == (pid + q) % every:
                # embassies first, then friendship with those we like
                if not D.has_embassy(g, pid, q) and g.civ_has(pid, U.EnablesEmbassies) and g.civ_has(q, U.EnablesEmbassies):
                    self.ex(g, pid, "open_negotiation", to=q, message="Let us exchange embassies.",
                            give=[{"type": "embassy"}], receive=[{"type": "embassy"}])
                elif not D.is_friends(g, pid, q) and D.opinion(g, pid, q) >= 0 and mine < theirs * P["friend_max_ratio"] \
                        and self.rng.random() < P["friend_chance"] - P["friend_chance_aggr"] * a:
                    self.ex(g, pid, "open_negotiation", to=q, message="Let us declare our friendship.",
                            give=[{"type": "declaration_of_friendship"}], receive=[])
                elif D.is_friends(g, pid, q) and rel.get("ra_until", 0) < g.turn \
                        and p.gold > D.ra_cost(g, pid, q) + P["ra_gold_margin"] and g.civ_has(pid, U.EnablesResearchAgreements):
                    self.ex(g, pid, "open_negotiation", to=q, message="A research agreement would benefit us both.",
                            give=[{"type": "research_agreement"}], receive=[])
            if g.turn > P["war_min_turn"] and not ctx["wars"] and pid not in self._war_prep and rel["treaty_until"] < g.turn \
                    and sum(ctx["threat"].values()) < mine * P["war_max_threat"] and not D.is_friends(g, pid, q):
                if mine > theirs * (P["war_power_ratio"] - P["war_power_ratio_aggr"] * a) \
                        and self.rng.random() < (P["war_chance"] + P["war_chance_aggr"] * a) * P["war_prep_rate"] \
                        and self._reachable_city(g, pid, q, ctx) is not None:
                    self._war_prep[pid] = {"player": q, "since": g.turn}
        prep = self._war_prep.get(pid)
        if prep and not ctx["wars"]:
            q = prep["player"]
            theirs = self.military_power(g, q)
            field = [u for u in ctx["military"] if _ud(g, u)["_domain"] == "Land" and g.military_at(u.idx) is u
                     and not (g.city_at(u.idx) and self._is_garrison(u))]
            target = self._reachable_city(g, pid, q, ctx) if g.player(q).alive else None
            rel = g.relation(pid, q) or {}
            if target is None or g.turn - prep["since"] > P["war_prep_timeout"] \
                    or mine < theirs * (P["war_abort_ratio"] - P["war_abort_ratio_aggr"] * a) \
                    or rel.get("treaty_until", 0) >= g.turn or D.is_friends(g, pid, q):
                self._war_prep.pop(pid, None)
            else:
                need = max(P["war_need_min"], min(P["war_need_max"], len(ctx["cities"]) // P["war_need_city_div"]
                                                  + P["war_need_base"]))
                if P["prep_gather"]:
                    if prep.get("target") != target.idx:
                        stage = min(ctx["cities"], key=lambda c: g.grid.distance(c.idx, target.idx))
                        prep["target"], prep["rally"] = target.idx, self._rally_point(g, pid, target.idx, stage.idx)
                    ready_units = [u for u in field if g.grid.distance(u.idx, prep["rally"]) <= P["prep_gather_radius"]]
                else:
                    ready_units = field
                overwhelming = len(ready_units) >= P["overwhelm_units"] and mine > theirs * P["overwhelm_ratio"]
                if (len(ready_units) >= need or overwhelming) \
                        and mine > theirs * (P["declare_ratio"] - P["declare_ratio_aggr"] * a):
                    if self.ex(g, pid, "declare_war", player_id=q, message="Your lands will be ours.") is not None:
                        gathered = P["prep_gather"]
                        self._war_plan[pid] = {"city": target.idx, "since": g.turn, "advance": gathered, "checked": -1,
                                               "rally": prep.get("rally")}
                    self._war_prep.pop(pid, None)
        elif prep:
            self._war_prep.pop(pid, None)
        k = max(1, int(P["lux_trade_every"]))
        if g.turn % k == pid % k:
            self.trade_luxuries(g, pid)

    def _reachable_city(self, g: Game, pid: int, q: int, ctx: dict):
        """A rival city this civilization could actually attack, given geography."""
        p = g.player(pid)
        cont = g.s.continents
        ours = {cont[c.idx] for c in ctx["cities"]}
        P = self.p
        overseas = P["war_overseas"] and ctx["cities"] and _can_embark(g, pid)
        near = [c for c in g.s.cities.values() if c.owner == q and p.explored[c.idx] and (overseas or cont[c.idx] in ours)]
        dist = {c.id: min(g.grid.distance(c.idx, mc.idx) for mc in ctx["cities"]) for c in near}
        near = [c for c in near if dist[c.id] <= P["war_target_max_dist"]]
        if not near:
            return None
        if P["war_target_pick"] == "nearest":
            return min(near, key=lambda c: (dist[c.id], c.pop))
        return min(near, key=lambda c: (c.pop, dist[c.id]))

    def trade_luxuries(self, g: Game, pid: int):
        """Offer luxury trades, which raise happiness on both sides."""
        from ..engine import economy
        P = self.p
        p = g.player(pid)
        mine = economy.luxury_resources(g, pid)
        surplus = [r for r, d in mine.items() if d["net"] >= P["lux_spare_at"]]
        if not surplus:
            if P["lux_buy"]:
                self._buy_luxury(g, pid, mine)
            return
        for q in p.met:
            other = g.player(q)
            if not other.alive or other.kind != "major" or g.at_war(pid, q):
                continue
            theirs = economy.luxury_resources(g, q)
            give = next((r for r in surplus if theirs[r]["net"] <= 0), None)
            if give is None:
                continue
            want = next((r for r, d in theirs.items() if d["net"] >= P["lux_spare_at"] and mine[r]["net"] <= 0), None)
            receive = [{"type": "resource", "resource": want, "amount": 1}] if want \
                else [{"type": "gold_per_turn", "amount": P["lux_sell_gpt"], "turns": g.speed["dealDuration"]}]
            if not want and g.player(q).gold < P["lux_buyer_min_gold"]:
                continue
            self.ex(g, pid, "open_negotiation", to=q, message="We have luxuries to spare. Shall we trade?",
                    give=[{"type": "resource", "resource": give, "amount": 1}], receive=receive)
            return
        if P["lux_buy"]:
            self._buy_luxury(g, pid, mine)

    def _buy_luxury(self, g: Game, pid: int, mine: dict):
        """While happiness is short, offer gold per turn for a luxury type we lack to a civ that has one to spare:
        a new luxury type is +4 happiness empire-wide, the cheapest happiness there is."""
        from ..engine import economy
        P = self.p
        p = g.player(pid)
        if economy.happiness(g, pid)["total"] >= len(g.player_cities(pid)) + P["lux_buy_hap_margin"]:
            return
        if economy.civ_stats(g, pid)["gold"] < P["lux_buy_gpt"]:
            return
        for q in p.met:
            other = g.player(q)
            if not other.alive or other.kind != "major" or g.at_war(pid, q):
                continue
            theirs = economy.luxury_resources(g, q)
            want = next((r for r, d in theirs.items() if d["net"] >= P["lux_spare_at"] and mine[r]["net"] <= 0), None)
            if want is None:
                continue
            if self.ex(g, pid, "open_negotiation", to=q, message="We would pay for a luxury you have to spare.",
                       give=[{"type": "gold_per_turn", "amount": P["lux_buy_gpt"], "turns": g.speed["dealDuration"]}],
                       receive=[{"type": "resource", "resource": want, "amount": 1}]) is not None:
                return

    def evaluate(self, g: Game, pid: int, other: int, give: list, receive: list) -> float:
        """Value a proposed deal from this civilization's point of view.

        Positive means the deal is worth accepting. The same function values both sides, so the bot's own
        offers are ones it would accept - which is what stops it proposing things nobody would take.
        """
        from ..engine import economy, research, diplomacy as D
        P = self.p
        mine, theirs = self.military_power(g, pid), self.military_power(g, other)
        dur = g.speed["dealDuration"]
        v = 0.0
        for items, sign in ((receive, 1), (give, -1)):
            for it in items:
                t = it["type"]
                if t == "gold":
                    v += sign * it["amount"]
                elif t == "gold_per_turn":
                    v += sign * it["amount"] * it.get("turns", dur) * P["deal_gpt"]
                elif t == "resource":
                    rd = g.rules.resources[it["resource"]]
                    if rd["resourceType"] == "Luxury":
                        have = economy.luxury_resources(g, pid)[it["resource"]]["net"]
                        v += sign * (P["deal_lux_needed"] if (sign > 0 and have <= 0) or (sign < 0 and have <= 1)
                                     else P["deal_lux_spare"]) * it.get("amount", 1)
                    else:
                        # strategic resources matter more as units need them: scale with the era
                        have = economy.strategic_resources(g, pid)[it["resource"]]["available"]
                        era = 1 + research.player_era(g, pid)
                        v += sign * (P["deal_strategic_needed"] + P["deal_strategic_needed_per_era"] * era if have <= 0
                                     else P["deal_strategic_spare_per_era"] * era) * it.get("amount", 1)
                elif t == "open_borders":
                    v += sign * P["deal_open_borders"]
                elif t == "embassy":
                    v += sign * P["deal_embassy"]
                elif t == "peace_treaty":
                    plan = self._war_plan.get(pid)
                    attacking = plan is not None and g.city_at(plan["city"]) is not None \
                        and g.city_at(plan["city"]).owner == other
                    if attacking and g.turn - plan["since"] < P["deal_peace_campaign_turns"] \
                            and mine > theirs * P["deal_peace_campaign_ratio"]:
                        v -= P["deal_peace_while_winning"]
                    else:
                        v += P["deal_peace"] if mine <= theirs * P["deal_peace_strong_ratio"] else -P["deal_peace_when_strong"]
                elif t in ("declaration_of_friendship", "defensive_pact"):
                    if sign > 0:   # mutual items appear on both sides; count once
                        v += P["deal_friendship"] if D.opinion(g, pid, other) >= 0 and pid not in self._war_prep \
                            else -P["deal_friendship_refuse"]
                elif t == "research_agreement":
                    if sign > 0:
                        v += P["deal_ra"] if g.player(pid).gold > D.ra_cost(g, pid, other) + P["deal_ra_margin"] \
                            else -P["deal_ra_refuse"]
                elif t == "declare_war":
                    v += sign * self._war_item_value(g, pid, it["target"], giving=sign < 0, mine=mine)
                elif t == "share_map":
                    v += sign * (P["deal_map_get"] if sign > 0 else P["deal_map_give"])
                elif t == "city":
                    c = g.city(it["city_id"])
                    v += sign * (P["deal_city"] + P["deal_city_per_pop"] * (c.pop if c else 1))
                elif t == "tech":
                    cost = research.tech_cost(g, pid, it["tech"])
                    v += sign * cost * (P["deal_tech_get"] if sign > 0 else P["deal_tech_give"])
        return v

    def _war_item_value(self, g: Game, pid: int, target: int, giving: bool, mine: float) -> float:
        """Worth of a "declare war on target" deal item. Giving one (we declare) costs the risk of fighting the
        target, the diplomatic price of attacking a city-state and the betrayal of a friend; receiving one (they
        declare) only helps against someone we fight or plan to fight."""
        from ..engine import diplomacy as D
        P = self.p
        prep = self._war_prep.get(pid) or {}
        ours = g.at_war(pid, target) or prep.get("target") in {c.idx for c in g.player_cities(target)} or \
            any(g.city_at(p["city"]) is not None and g.city_at(p["city"]).owner == target
                for p in [self._war_plan.get(pid)] if p)
        if not giving:
            return P["deal_ally_war"] if ours else P["deal_other_war"]
        if g.at_war(pid, target):
            return 0
        theirs = self.military_power(g, target)
        cost = P["deal_war_cost"] + P["deal_war_risk"] * max(0.0, theirs / max(1.0, mine) - P["deal_war_risk_from"])
        if g.player(target).kind == "city_state":
            cost += P["deal_war_city_state"]      # other city-states and the target's protectors take offence
        if D.is_friends(g, pid, target) or D.has_pact(g, pid, target):
            cost += P["deal_war_betrayal"]
        if ours:
            cost *= P["deal_war_planned_share"]   # we meant to fight them anyway
        return cost * (P["deal_war_mult"] - P["deal_war_mult_aggr"] * self.aggression)

    def handle_negotiations(self, g: Game, pid: int):
        """Answer every negotiation waiting on this civilization."""
        for n in list(g.s.negotiations):
            if n["status"] == "open" and n["awaiting"] == pid:
                self.respond(g, pid, n["id"])

    def respond(self, g: Game, pid: int, nid: int):
        """Accept, reject or counter one negotiation."""
        from ..engine.diplomacy import get_negotiation
        P = self.p
        n = get_negotiation(g, nid)
        if n["status"] != "open" or n["awaiting"] != pid:
            return
        other = n["responder"] if pid == n["initiator"] else n["initiator"]
        if not n["proposal"]:
            if n["exchanges"] >= 3:
                self.ex(g, pid, "respond_negotiation", negotiation_id=nid, action="reject",
                        message="We have nothing further to discuss.")
            else:
                self.ex(g, pid, "respond_negotiation", negotiation_id=nid, action="reply",
                        message="Words are wind. Make a concrete proposal.")
            return
        if n["proposal_by"] == pid:
            self.ex(g, pid, "respond_negotiation", negotiation_id=nid, action="reply", message="Our offer stands.")
            return
        give = n["proposal"].get(str(pid), [])
        receive = n["proposal"].get(str(other), [])
        value = self.evaluate(g, pid, other, give, receive)
        if value >= 0:
            if self.ex(g, pid, "respond_negotiation", negotiation_id=nid, action="accept", message="Agreed.") is None:
                self.ex(g, pid, "respond_negotiation", negotiation_id=nid, action="reject",
                        message="We cannot fulfil those terms.")
        elif n["exchanges"] < P["counter_rounds"] and value > -P["counter_max_gap"] and g.player(other).gold >= -value:
            ask = int(-value) + P["counter_margin"]
            if self.ex(g, pid, "respond_negotiation", negotiation_id=nid, action="counter",
                       message=f"Add {ask} gold and we have a deal.", give=give,
                       receive=receive + [{"type": "gold", "amount": ask}]) is None:
                # a counter the rules refuse (we can't pay what they asked, say) must still end our move, or the
                # negotiation stays open waiting on us until it times out
                self.ex(g, pid, "respond_negotiation", negotiation_id=nid, action="reject",
                        message="That does not interest us.")
        else:
            self.ex(g, pid, "respond_negotiation", negotiation_id=nid, action="reject", message="That does not interest us.")
