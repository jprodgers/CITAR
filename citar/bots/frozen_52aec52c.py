# Frozen copy of citar/bots/basic.py made by citar.lab on 2026-09-18 11:24
...
DEFAULT_PARAMS: dict = {
    ...
    "prod_mode": "classic",
    "u_food": 3.6, "u_production": 2.0, "u_gold": 0.67, "u_science": 1.0, "u_culture": 1.0, "u_faith": 1.0,
    "u_happiness": 1.0, "u_happiness_low": 3.0,     # x3 while happiness < 10 or < number of cities (UnCiv)
    "u_settler": 30.0, "u_worker": 1.0, "u_military": 1.0, "u_offense": 3.0, "u_wonder_bonus": 4.0,
    "lux_trade_every": 7,         # turns between luxury-trade offers
}
...
class BasicBot:
    ...
    def _choose_production_unciv(self, g: Game, pid: int, c, ctx: dict, counts: dict, army: int, danger: bool):
        from ..engine import cities as cm, economy
        ...
        if worker:
            want = 1 if n <= 1 else (1.8 * n if n <= 5 else 9 + 1.3 * (n - 5))
        ...
        # --- buildings ------------------------------------------------------------------------------------
        wonders = items["wonders"] if (over_avg and total_pop >= 12 and not c.puppet) else []
