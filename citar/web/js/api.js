...
export const api = {
  ...
  botProfiles: () => request("GET", "/api/bots/profiles"),
  botProfile: (id) => request("GET", `/api/bots/profiles/${encodeURIComponent(id)}`),
  saveBotProfile: (id, body) => request("PUT", `/api/bots/profiles/${encodeURIComponent(id)}`, body),
  createBotProfile: (body) => request("POST", "/api/bots/profiles", body),
  forkBotProfile: (id, name) => request("POST", `/api/bots/profiles/${encodeURIComponent(id)}/fork`, { name }),
  deleteBotProfile: (id) => request("DELETE", `/api/bots/profiles/${encodeURIComponent(id)}`),
  botEngines: () => request("GET", "/api/bots/engines"),
  botSchema: (engine) => request("GET", `/api/bots/schema?engine=${encodeURIComponent(engine)}`),
  botRankings: (factorial = false) => request("GET", `/api/bots/rankings?factorial=${factorial ? 1 : 0}`),
  queueBotExperiment: (body) => request("POST", "/api/bots/experiments", body),
