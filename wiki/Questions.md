...
measurement too — see [reading a result honestly](Benchmarks#reading-a-result-honestly).
...
Over the internet, run a [server](Deploying-a-server): accounts, invitations, single sign-on,
...
vision, or set up a [benchmark suite](Benchmarks) and have every model play the same map.
...
- and [probes](Scenarios-and-probes#probes) exist for testing one decision instead of a whole game.
...
`scripts/import_unciv.py` at a checkout and regenerate. See [MODDING.md](Modding).
...
`GET /api/tools` lists every tool with its JSON schema. See [API.md](HTTP-and-tool-API).
...
backup timer and a documented restore drill — [server/RUNBOOK.md](Runbook).
...
*This page is generated from [`docs/FAQ.md`](https://github.com/jprodgers/CITAR/blob/main/docs/FAQ.md) and any edit made here will be overwritten. Corrections are welcome as a pull request.*
